...
## Current Release — v1.51.7 (2026-09-11)

The newest annotated tag is `v1.51.7` at `96fdbde` ("Release v1.51.7 Sound-gate improved Whisperer agent, and avatar."), which is also local `main` and `HEAD`, so source-mode resolution reports the bare `1.51.7` with no post-tag boundary. Source truth is **88 workflow agents**, **66 wrapped chat-agent launchers**, **108 built-in Multi-Turn tools** (20 core + 66 wrapped + 12 ACPX/Skill + 10 External-MCP supervisors), **105 root stdio MCP tools**, **29 skills**, and **200 migrations**.
