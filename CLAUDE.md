...
## Current Release — v1.51.5 (2026-09-10)

The newest annotated tag is `v1.51.5` at `4a7f1cb` ("Release v1.51.5 Improving welcome page default key and making a shield into console window in frozen mode"), which is also local `main`, `origin/main` and `origin/HEAD`, so source-mode resolution reports the bare `1.51.5` with no post-tag boundary. Source truth is **88 workflow agents**, **66 wrapped chat-agent launchers**, **108 built-in Multi-Turn tools** (20 core + 66 wrapped + 12 ACPX/Skill + 10 External-MCP supervisors), **105 root stdio MCP tools**, **29 skills**, and **199 migrations**.
