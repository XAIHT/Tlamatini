...
## Current Release — v1.63.0 (2026-09-19)

The current annotated release tag is `v1.63.0` at `2cf8e7f`, created 2026-09-19. Runtime/build resolution reports bare `1.63.0` for this tag and subsequent source commits, without a `.devN`, `+gSHA` or `.dirty` suffix. The current source adds Video-Analyzer audio-track transcription and audiovisual summaries through `analysis_type`, retaining `robotics` as the default and exposing the new modes through Parametrizer, workflows and MCP. Preceding releases include `v1.62.2`, `v1.62.0` (the PDF canvas) and `v1.61.0`. The three tags between it and `v1.51.9` are `v1.52.0` (PPTXer), `v1.52.2` (PDFer + PPTXer styles) and `v1.52.3` (LaTeXer styles). Source truth is **89 workflow agents**, **67 wrapped chat-agent launchers**, **109 built-in Multi-Turn tools** (20 core + 67 wrapped + 12 ACPX/Skill + 10 External-MCP supervisors), **105 root stdio MCP tools**, **29 skills**, and **207 migrations**.
