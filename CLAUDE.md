...
│   │   │   ├── pptxer/             # PPTXer — DECK COMPOSER, the third member of the document family (PDFer COMPOSES a PDF, LaTeXer TYPESETS one, PPTXer BUILDS the slides). Tlamatini's answer / Markdown / an outline → ONE finished .pptx with embedded images (including images FETCHED FROM THE INTERNET), video, audio, native editable diagrams, real tables and generated artwork. ZERO new deps (python-pptx + Pillow already ship). Built above all for MARKETING and GAMING: 24 nuances, EIGHT of them gaming and EIGHT marketing, and every typeface installed on the machine — Terminator, Nasalization, Verdana, Times New Roman, Courier — resolved through 15 pairings. ⚠️ **Font resolution is DETERMINISTIC and WIDTH-AWARE by contract (2026-09-14)**: the scan iterates `sorted(set(paths))` (a bare `set` made the winning file change per process, so the same deck audited clean once and overflowed the next), and a width in the STYLE field is folded into the FAMILY (`("Arial","Narrow")` → `"Arial Narrow"`, which stopped Arial Narrow evicting real `arial.ttf` and being served as "Arial" with `substituted=False`). See docs/claude/agents.md → PPTXer. ⚠️ **IT LOOKS AT WHAT IT WROTE**: python-pptx does NO layout, so a clean save proves nothing. Every element is placed from REAL font metrics (overlap is prevented by construction, not detected afterwards), then the finished deck is RENDERED — through the installed PowerPoint via COM when there is one — and those pixels are MEASURED for overlap, off-slide ink, overflowing text and true contrast. `ground_truth` is True only when PowerPoint's own renderer produced them; a deck with measured defects reports `created_with_findings` and never claims to be clean. 14 flat sibling modules (pptxer_color/fonts/nuance/layout/theme/draw/media/render/docmodel/audit/build/diagram; siblings NOT a package, because the agent runs as `python pptxer.py` from a copied runtime dir). action: create|outline|render|audit|info|fonts|validate (canvas + chat_agent_pptxer)
