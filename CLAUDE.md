...
## Current Release — v1.62.2 (2026-09-16)

The newest annotated tag is `v1.62.2` at `92a5830` ("Release v1.62.2 Setting more comfortable value of sleep stop timer to 3.5s, and setting jcyhsiao/qwen3.5cloud:latest to replace the obsolete qwen3.5:cloud."), created 2026-09-16. Local `main` is **two commits past** that tag (`5275f44`), so `git describe --tags` reports `v1.62.2-2-g5275f44` while version resolution still reports plain `1.62.2` - Tlamatini never appends a `.devN`, `+gSHA` or `.dirty` suffix. The intervening tags are `v1.61.0` and `v1.62.0` (the PDF canvas). The three tags between it and `v1.51.9` are `v1.52.0` (PPTXer), `v1.52.2` (PDFer + PPTXer styles) and `v1.52.3` (LaTeXer styles). Source truth is **89 workflow agents**, **67 wrapped chat-agent launchers**, **109 built-in Multi-Turn tools** (20 core + 67 wrapped + 12 ACPX/Skill + 10 External-MCP supervisors), **105 root stdio MCP tools**, **29 skills**, and **204 migrations**.
