...
- A **Visual Agentic Workflow Designer** (ACP) with 89 drag-and-drop agent types
...
│   │   ├── agents/                 # 89 workflow agent templates
│   │   │   ├── flowcreator/
│   │   │   ├── flowhypervisor/
...
│   │   │   ├── esp32er/            # ESP32 firmware bridge — direct PlatformIO `pio` CLI (no MCP server), zero-config get-platformio.py auto-bootstrap + fail-safe preflight. **SHIPS `ESP32TemplateProject/` (2026-09-15)** — `create_project` COPIES it and stamps `board`/`framework` into platformio.ini, the same uniform template-project scheme as Arduiner. ⚠️ It used to run a BARE `pio project init`, which leaves `src/` EMPTY: under the Arduino framework the next `build` then failed to link (`undefined reference to 'setup'`), so a freshly scaffolded project DID NOT COMPILE. `_MINIMAL_MAIN_CPP` keeps that guarantee even if the template is missing — create_project must NEVER return a project that cannot build. Verified: `pio run` → SUCCESS in 15.08 s on PlatformIO 6.1.19 (canvas + chat_agent_esp32er)
│   │   │   ├── esphomer/           # ESPHome smart-home device bridge — direct `esphome` CLI (no MCP server), YAML device configs (NO C++), zero-config `pip install esphome` auto-bootstrap + fail-safe preflight + headless new_config generator. **`ESPHomeTemplateProject/` IS NOW REACHABLE (2026-09-15)** — it shipped and was tracked in git but NO CODE PATH EVER READ IT (`new_config` generates from the inline `_DEVICE_YAML_TEMPLATE`), so the shipped sample and the generated output could drift apart unnoticed; NEW `action: scaffold_template` copies it, and it grew 2 → 6 files (a self-contained light device, a `packages:`+`!secret` sensor device, `common/base.yaml`, `secrets.yaml.example`, `.gitignore`). ⚠️ NEW `use_secrets` DEFAULTS FALSE and must stay so — `!secret` is a custom YAML tag that a plain `yaml.safe_load` cannot read; `_write_secrets_file` never overwrites an existing secrets.yaml (canvas + chat_agent_esphomer)
...
│   │   │   ├── video_analyzer/       # Video-Analyzer — "eye" of Robotic-Loop-Training: watches a recorded video and rules PASS_OK / FAIL_NO_MOTION / FAIL_WRONG_MOTION / UNCLEAR via a deterministic OpenCV motion gate + triple-model Ollama CLOUD vision (gemma4:cloud ∥ jcyhsiao/qwen3.5cloud:latest → glm-5.3:cloud merge; PASS only if both agree); emits INI_SECTION_VIDEO_ANALYZER + a substring-safe TLM_VERDICT:: line a Forker branches on (canvas + chat_agent_video_analyzer); also analysis_type=transcription (video audio tracks) or summary (visual evidence + speech), TLM_ANALYSIS routing and report artifacts; see agents/video_analyzer/README.md
...
## PDFer — 24 visual styles, semantic design and measured layouts (2026-09-15)

PDFer composes PDFs from Markdown, HTML, plain text and images, and merges or inspects existing PDFs. **24 explicit visual identities in five families** now sit alongside the **20 content themes**. The [signature collection](Tlamatini/agent/agents/pdfer/STYLES.md) is the canonical catalog, configuration reference and verification guide.

**Keep purpose and appearance separate.** `nuance` selects the document's semantic treatment, or detects it when empty. `style` selects playful/nursery, cyberpunk, cosmic, electronics or Tlamatini appearance; `auto` retains the existing semantic design. Names and aliases match exactly after normalization, never by fuzzy inference from source prose. Unknown styles retain the semantic theme and produce a diagnostic. Existing nuance aliases keep priority: `nuance: astronomy` still means `scientific_dark`, while `style: astronomy` means `astral_atlas`.

**Catalog and flow contract.** `mode: styles` lists IDs, labels, families and aliases with `status: inspected`; it writes no PDF. Other modes remain `auto|markdown|html|text|images|mixed|merge|info|validate`. Atelier renders report `style` and `style_family` alongside the existing `nuance`, `palette`, `layout_clean`, `repairs` and operation fields. Keep `pdfer.py`, `chat_agent_registry.py`, `config.yaml`, `agent_contracts._PARAMETRIZER_OUTPUT_FIELDS['pdfer']` and FlowCreator's reference aligned. Existing field names remain valid.

**Twelve flat helper siblings** live beside `pdfer.py`: `pdfer_color`, `pdfer_typography`, `pdfer_nuance`, `pdfer_theme`, `pdfer_ornament`, `pdfer_docmodel`, `pdfer_tables`, `pdfer_atelier`, `pdfer_audit`, `pdfer_consult`, plus **`pdfer_styles`** (the 24-style registry and palette/type defaults) and **`pdfer_artwork`** (original deterministic ReportLab vector art). Copy the complete template directory to a pool: it runs as `python pdfer.py` without a Django package context. The existing grouped atelier import can fall back to legacy rendering; the style catalog itself needs its style module.

**Layout and design contracts:**

1. Measure against the actual registered font and available frame. Table widths remain bounded; the last repair rung is `split_columns`. A 0.75-point measurement allowance prevents ordinary words breaking because the renderer and solver round widths differently. Unbroken tokens wrap without injected spaces.
2. Cover title/subtitle/author are measured before placement. An opaque reading area separates text from artwork. Long title/subtitle content continues onto later pages; only a repeated footer note may be shortened with an ellipsis after reserving the measured folio width. Small pages retain a usable frame.
3. Apply palette validation after style, seed and per-role overrides. Muted text, H3/H4, quotes, captions, footers and alternating table rows use the normal-text 4.5:1 floor; larger headings use their applicable floor. Check cover ink against its actual opaque background. Sparse host font families must not resolve bold text to an italic file.
4. **A style never raises the content's decoration ceiling.** Legal, clinical and financial treatments keep no ornament; confidence and precision constraints still apply. `decorations` can only lower the ceiling. Signature cover art occupies a clipped, text-free region when rich decoration is permitted.
5. `predominant_color` recolors the palette; explicit per-role/font/page settings take precedence. `background_mode` can change the ground while retaining style identity. `style: auto` preserves existing behavior.
6. Use stable SHA-256 seeds for artwork. Legacy raster cache keys include all palette roles and raster scale; the new cover art remains vector. Host font availability can still change the resolved typeface across machines.
7. `engine: auto` uses atelier unless custom `css` selects the legacy renderer. Signature styling does not redesign legacy CSS, image-only pages or existing merged PDFs.
8. `layout_audit: true` is the default. It reopens the finished PDF and measures glyph overlap, off-sheet ink, blank pages and contrast. `bleeds` and `frame_intrusions` remain distinct. **A saved PDF or `status: created` alone is not proof of a clean layout**; a blank `layout_clean` is not a pass. Report findings and repairs honestly.
9. `ollama_polish` and `ollama_design` remain off by default; unavailable optional services fall back to the existing content/design. The new styles add no runtime dependencies or downloads.

**Dated verification.** On 2026-09-15, the three PDFer suites passed **130 tests**; all **24 two-page style samples (48 pages)** had clean audits with no overlaps, bleeds, blank pages or contrast findings. Coverage includes A4 portrait/A5 landscape, long titles and subtitles, footer fitting, palette overrides, alias compatibility and reproducible artwork. These results describe those fixtures and the tested environment, not every possible document or an installed-build verification. The earlier 2026-09-06 audit recorded 22 documents/92 pages after the original table repair; its history remains in [recent fixes](docs/claude/recent-fixes.md).

**Keep generated previews disposable.** `scripts/verify_pdfer_styles.py` reproduces the gallery, per-style PDFs/PNGs and audit report under ignored `artifacts/pdfer-styles/`, plus ignored `output/pdf/pdfer-style-atlas.pdf`. They were removed after inspection. Keep the generator, source, tests and guide; do not commit regenerated preview assets.
...
## 🔇 Whisperer LISTENS — the silence gate is the default (2026-09-11)

**Angela's ask:** *"By default operate while recording based on a sound detection gate … when a silence of more than 10 seconds (configurable) is detected then stop recording"*, with the countdown visible in the console bar that already appears.

**`record_seconds` IS THE MODE SWITCH.** `0` (the new default) = no duration was given, so keep recording while the speaker is still talking; any number = record exactly that long with the gate OFF. This is the idiom already sitting one line above it in the same `config.yaml` (`sample_rate: 0` = the native rate). Companions: `silence_timeout_seconds` (**3.5** since 2026-09-18; it shipped at 10), `silence_gate` (auto/on/off), `max_record_seconds` (300 ceiling), `silence_threshold_db` (0 = measure the room).

**CONTRACTS — do NOT weaken:**

1. **The noise floor STARTS pessimistically low**, so the first threshold is the absolute **−50 dBFS** floor and somebody who begins talking on sample one is heard. A gate that "listens to the room first" mistakes that speaker's own voice for the room and gates them out — **the worst failure available here**, because it cuts a person off mid-sentence and reads as a crash.
2. **Unbroken "voice" for 15 s re-baselines the room** to the quietest block of that stretch. A steady −45 dBFS fan sits ABOVE the starting threshold, so without this the gate would never fire at all. People breathe; fans do not.
3. **The floor is never updated from a block we believe is speech**, or a long sentence raises the floor over its own voice.
4. **`SilenceGate.feed()` runs in the PortAudio callback**: plain float arithmetic only — no lock, no logging, no allocation, no disk. A callback that blocks drops samples, and dropped samples are a wrong transcript.
5. **The blocking `sd.rec` fallback CANNOT gate** (no callback) — it records a fixed length and **SAYS SO** (`stop_reason: fixed_fallback`). Silently recording 30 s and still calling it gated is the plausible-but-wrong deliverable this agent must never produce.
6. **`duration_seconds` is the length ACTUALLY captured**, never the length requested.
7. **The 4 new `INI_SECTION_WHISPERER` fields were APPENDED, never renamed** (`capture_mode`, `stop_reason`, `silence_timeout_seconds`, `speech_seconds`), kept in step with `agent_contracts._PARAMETRIZER_OUTPUT_FIELDS['whisperer']`.
8. **In fixed mode the painted console line is byte-for-byte what it always was.** Only a gated run grows the silence bar.

**⚠️ THE HIGHEST RISK IS THE MODEL'S HABIT, NOT THE CODE.** The tool description used to say *"records record_seconds (default 30)"*, so Tlamatini would keep volunteering a duration, the gate would never fire, and the feature would be invisible **while looking like it worked**. It now says **DO NOT PASS record_seconds UNLESS THE USER NAMED A DURATION**, pinned by `test_tool_description_tells_the_model_not_to_volunteer_a_duration`. **A FlowHypervisor note matters too**: a gated Whisperer waiting for a human is silent and healthy for up to 5 minutes — `monitoring-prompt.pmt` → *WHISPERER SPECIAL NOTES* tells the watchdog not to flag it. Full contract: `docs/claude/agents.md` → *Whisperer*; story: `docs/claude/recent-fixes.md` (2026-09-11); coverage `agent/test_whisperer_agent.py` (69 tests).

---

## Current Release — v1.65.4 (2026-09-21)

The current published release is **`v1.65.4`** — annotated 2026-09-21, peeling to commit `b09c4ff`, and the **Latest** release on GitHub. Runtime/build resolution reports bare `1.65.4` for this tag and subsequent source commits, without a `.devN`, `+gSHA` or `.dirty` suffix. ⚠️ **A later `v1.65.5` tag exists in the repository with NO published release attached**, so `git describe` (and `scripts/skill_inventory.py`) report `v1.65.5` as the nearest tag while the shipped release is `v1.65.4` — read the release list, not just the nearest tag, before writing a "current release" line. The `1.65.x` line is the **context** line: `v1.65.0` implemented the context meter, `v1.65.2` improved the Context Governor so it measures the real request and never binds zero tools, `v1.65.3` repaired LaTeXer (a leading brace before a command was being eaten as an optional argument), and `v1.65.4` improved the context gauge itself. Preceding releases include `v1.64.0` (Video-Analyzer audio-track transcription and audiovisual summaries through `analysis_type`, retaining `robotics` as the default and exposing the new modes through Parametrizer, workflows and MCP), `v1.62.2`, `v1.62.0` (the PDF canvas) and `v1.61.0`. The three tags between it and `v1.51.9` are `v1.52.0` (PPTXer), `v1.52.2` (PDFer + PPTXer styles) and `v1.52.3` (LaTeXer styles). Source truth is **89 workflow agents**, **67 wrapped chat-agent launchers**, **109 built-in Multi-Turn tools** (20 core + 67 wrapped + 12 ACPX/Skill + 10 External-MCP supervisors), **105 root stdio MCP tools**, **29 skills**, and **208 migrations**.

**The carried v1.60.0 line enhances the entire chain of visual agents.** The document family gained a third member — **PPTXer**, the editable-deck composer (36 named treatments = 24 automatic content treatments + 12 explicit visual styles, 17 font pairings, measured full-text pagination that continues long content onto a new slide instead of truncating it, and a native-render audit) — while **PDFer** gained **24** explicit visual identities in five families alongside its 20 semantic themes, and **LaTeXer** gained **30** signature styles in six families, discoverable with `action: list_styles` (no TeX installed required). Alongside them the desktop-control and flow contracts were tightened: Mouser resolves explicit physical, window and screenshot coordinates; Keyboarder binds Unicode/key delivery to a verified window; Shoter publishes its capture geometry; Parametrizer derives its parser registry from the live contracts and validates complete typed mappings; and FlowCreator selects from all 89 installed agents and validates the generated graph before publishing. ⚠️ **`input_sent` is delivery evidence, not proof the application accepted the input** — a failed or interrupted input segment must be observed before replay. Contracts: `docs/desktop-input-and-flow-contracts.md`, `docs/agent-coverage.md`, and the three `STYLES.md` guides.

**The carried v1.51.9 tag repairs the Ollama sampler, opens the Catalog with VOICE COMMANDS, and makes the uninstaller refuse to destroy the user's own work.** `ollama_repeat_penalty` shipped at **1.9** — roughly double any documented ceiling — and it was emptying answers: these are reasoning models, so pushed that hard they spend the whole turn thinking and return empty `content` (measured at **196,003** and once **3,101,216** characters of thinking). It now ships at **1.2**, `ollama_repeat_last_n` is finally WIRED (it had been unsent for months, so setting it did nothing) at **256**, and `ollama_num_ctx` reads the real **1,048,576** instead of a number that described nothing — with the caveat that ⚠️ **this failure is HEAVY-TAILED and must never be tuned from one run** (one 6-trial cell scored 0/6, eight trials of the same cell scored 7/8, at `temperature` 0.0). Full contract: `docs/claude/architecture.md` → *Ollama sampler settings*. Alongside it the Catalog of Prompts gained a **VOICE COMMANDS** section at index 0 (ahead of Getting Started) turning the sound gate into a user-facing feature — you speak, Whisperer transcribes, and the transcript becomes the prompt; the **uninstaller** now refuses to run while Tlamatini is still up and never deletes the user's own content; and `chat_agent_run_wait` stopped burning its entire timeout instead of returning the moment the run finished.

**v1.51.7 makes Whisperer LISTEN instead of counting seconds, and gives the avatar a real mouth.** Whisperer used to record a flat `record_seconds: 30`, which cuts a sentence in half or leaves you listening to an empty room. The default is now a **SOUND GATE**: it keeps recording while you are still talking and stops on its own after a window of silence (`silence_timeout_seconds` — it shipped at 10 s and is **3.5 s** since 2026-09-18), bounded by a 300-second ceiling. The obstacle was never the audio — it was telling *"no duration given"* from *"you asked for 30 seconds"*, since `record_seconds` always held a number; solved with the idiom already one line above it in the same config, so **`record_seconds: 0` means "listen until the speaker stops"** exactly as `sample_rate: 0` means the native rate, and any number you name turns the gate off and is honoured to the sample. `SilenceGate` is fed one RMS per ~20 ms block from INSIDE the PortAudio callback (plain float arithmetic — no lock, no logging, no allocation: a callback that blocks drops samples, and dropped samples are a wrong transcript), tracking the room with an asymmetric noise floor that starts deliberately sensitive so **a speaker who begins talking on sample one is heard** — a gate that "listens to the room first" mistakes that speaker for the room and cuts them off mid-sentence — while unbroken "voice" for 15 s re-baselines the room, because a steady fan is not a person. The console REC bar grew a live silence countdown, `duration_seconds` now reports what was actually captured rather than what was requested, and the blocking `sd.rec` fallback cannot gate so it **says so** instead of quietly pretending. The highest risk was never the code but the model's habit of volunteering `record_seconds`, so the tool description now forbids it unless the user named a duration, pinned by a test. Alongside it the chat **avatar** replaced a `setInterval(…, 150)` metronome with a 2-D canvas compositor whose eyes and mouth are continuous values blending the same four portraits, driven by `speechSynthesis` boundary events — no GPU, no LLM, 74/74 visible checks. Contracts: `docs/claude/agents.md` → *Whisperer* and `docs/claude/recent-fixes.md` (2026-09-11); coverage `agent/test_whisperer_agent.py` (**69 tests**).

**v1.51.5 shields the console and gives the welcome page a default key.** Windows consoles ship with QuickEdit ON, so a click or drag inside the window put the console into selection mode and **`WriteConsoleW` stopped returning** — and because a block is not an exception, the `try/except` around the write could never catch it, which is why the symptom read as a *hang*. Worse, `_TeeStream.write()` wrote the console FIRST and `tlamatini.log` SECOND, so one mouse click froze the calling thread, the durable log itself, and then every other thread as each reached the same line. Two layered defences: `console_quick_edit: false` (frozen builds only, because console mode outlives a source-mode run and belongs to the developer's own terminal) and `_ConsoleWriter`, a bounded queue drained by one daemon thread in BOTH modes — the log is written before the console is queued, drops are never silent, and Ctrl+C is forced on, verified and rolled back on doubt. Alongside it, `welcome_enter_default.js` makes **Enter** open the chat straight from the post-login page, in two layers, deferring to `document.activeElement` so a user who tabbed to *Logout* is never sent to the chat instead. Contracts: `docs/claude/architecture.md` → *The CONSOLE SHIELD* and `docs/claude/frontend.md` → *Welcome page*; coverage `agent/test_console_shield.py` (28 tests).
...
- **Agents** — creating a new agent (8-step), naming conventions, lifecycle, all 89 agent types, FlowCreator, FlowHypervisor: @docs/claude/agents.md
...
│   │   │   ├── pptxer/             # PPTXer — editable deck composer; 36 treatments (24 automatic + 12 explicit via nuance), 17 font pairings, measured full-text pagination, saved/native-render audits, atomic saves, bounded Office worker. Flat sibling modules travel with pool copies. See pptxer/STYLES.md.
│   │   │   ├── pdfer/              # PDFer document composer; 24 explicit visual styles + 20 semantic themes. Twelve flat helper siblings include pdfer_styles and pdfer_artwork. mode: auto|markdown|html|text|images|mixed|merge|info|validate|styles; catalog is read-only. Atelier measures text/tables, draws deterministic vector art within semantic limits and reports layout_clean; style/style_family are flow fields. See STYLES.md; no new runtime dependencies. Exec Report + Ask-Execs tier A.
│   │   │   ├── latexer/            # LaTeXer — LaTeX TYPESETTING, the typesetting sibling of PDFer (PDFer COMPOSES from Markdown/HTML/images; LaTeXer TYPESETS from .tex: real maths, bibliographies, cross-refs, index). Embeds the WHOLE mcp-latex-server surface NATIVELY (create/template/edit/read/list/validate/structure/compile) — NO MCP server, NO sidecar, NO new dependency (stdlib only: subprocess+shutil+glob+re+urllib) — PLUS whole-PROJECT compile of a .tex SET (master auto-detected, \input followed), a real BibTeX/Biber + makeindex + makeglossaries convergence loop, latexmk pass-through, and LaTeX-log diagnostics a human can read. **Compilation needs a TeX distribution; MiKTeX is recommended** (https://miktex.org/download) — Tlamatini bundles NO TeX distribution (several GB; the release must stay <2 GB); MiKTeX is preferred because `--enable-installer` installs a missing .sty ON DEMAND mid-compile, when the package is available. ⚠️ latexmk is probed for USABILITY not presence (it ships with MiKTeX but is a PERL script; most Windows boxes have no Perl → auto-fallback to the built-in loop). action: compile|compile_project|scaffold_compile|create_file|create_from_template|edit_file|read_file|list_files|validate_tex|structure|clean|validate|install|list_styles; auto_preamble wraps a bare fragment; shell_escape OFF by default (\write18 = RCE); saves to Documents/TlamatiniLaTeX, projects to <app>/Templates/LaTeXer; fail-safe preflight REFUSES rather than mis-typeset; **EIGHT-RUNG REPAIR LADDER (v1.48.2) so a failed build self-heals — lint → preamble → rules → log_directed → acquire → engine_swap → model → bisect, each repair applied to a COPY and re-linted (a repair that worsens the lint is REVERTED), the author's file untouched unless `repair_write_back`, every rung audit-traced, quarantined blocks named; ⚠️ the DESTRUCTIVE `bisect` rung is strictly LAST (reordered 2026-08-05) — do NOT swap it back ahead of `model`**; a DEGRADED build never claims clean success; INI_SECTION_LATEXER; Exec Report + Ask-Execs tier A (canvas + chat_agent_latexer) 30 opt-in styles, independent templates, engine-free list_styles; flat siblings latexer_styles.py, latexer_artwork.py, latexer_design.py; see STYLES.md.
