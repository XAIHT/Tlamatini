...
## Current Release — v1.51.9 (2026-09-14, annotated)

The newest annotated tag is `v1.51.9` at `cb30edf` ("Release v1.51.9 Improved the Flow-chain parameters, some general fixes, and avoiding deletion of user-elements."); local `main` and `HEAD` sit one commit past it, which changes nothing because Tlamatini emits the bare base tag without a distance or dirty suffix — runtime still resolves the reachable bare release tag through Git/build metadata. It repairs the **Ollama sampler** (`ollama_repeat_penalty` 1.9 → **1.2**, which was emptying answers by making reasoning models think forever; `ollama_repeat_last_n` finally WIRED at **256** after months of being silently unsent; `ollama_num_ctx` raised to the real **1,048,576**), opens the Catalog of Prompts with a **VOICE COMMANDS** section so a spoken sentence becomes the prompt, makes the **uninstaller** refuse to run while Tlamatini is up and never delete the user's own content, and stops `chat_agent_run_wait` burning its whole timeout. The carried **v1.51.7** tag makes **Whisperer LISTEN** rather than count seconds: with no duration given a sound gate keeps recording while the speaker is still talking and stops after 10 s of silence (`record_seconds: 0` is the mode switch, exactly as `sample_rate: 0` means the native rate), with the countdown live in the console REC bar, and it gives the chat avatar a real talking mouth driven by speech-boundary events instead of a fixed metronome. Source truth is **89 workflow agents**, **67 wrapped chat-agent launchers**, **109 built-in Multi-Turn tools** (20 core + 67 wrapped + 12 ACPX/Skill + 10 External-MCP supervisors), **105 root stdio MCP tools**, **29 skills**, and **204 migrations**.
