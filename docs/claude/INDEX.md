...
- **acpx.md** — ACPX (Agent Communication Protocol eXtension): authoritative definition, the 14-agent registry with transport profiles, all 12 LLM-facing tools, canonical flows (spawn-and-go, multi-CLI relay, harvest-transcript, skill-routing), runtime drain mechanics, permission model, and the "when the user says ACPX" decision matrix.
