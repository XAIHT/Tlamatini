...
User overrides go in `config.json`, and since v1.51.2 they can retune **the whole spec**, not just the command:
...
## The delivery verdict — an exit code is ONE BIT, and it lies (v1.51.2)
...
- `services/agent_contracts.py` declares ACPXer's structured fields; the generated catalog supplies Parametrizer parser membership, so ACPXer output can feed a downstream node's config. Regenerate with `python scripts/update_flow_catalog.py` after changing the contract.
