...
- **Runtimes are PROVISIONED, not assumed** (2026-08-15). Because most servers are `npx -y <pkg>` / `uvx <pkg>`, `agent/runtime_provisioner.py` keeps Tlamatini's OWN `node`/`npm`/`npx`/`pnpm`/`uv`/`uvx` in `%LOCALAPPDATA%\Tlamatini\runtimes`, downloaded once on demand from the official upstreams with sha256 verification and an atomic install — no admin, no system-PATH change, nothing bundled into the installer. Resolution order: `<tool>_executable` config → the private runtime → system PATH → well-known per-user locations, so **an existing system Node always wins**. `_StdioMcpClient` spawns through `augment_env()` + `resolve_spawn()`, the latter rewriting Windows' `npx.cmd` shim to `node.exe <npx-cli.js>`; `_connect` provisions a missing manager on the background connect thread before spawning. Toggle with `runtime_autoprovision` (default `true`).
