...
| **Notifier** | Browser popup + sound AND a native Windows toast (Action Center) on pattern detection (LangGraph). |
...
git tag -a v1.10.0 -m "Release 1.10.0: <one-liner>"   # annotated tag
...
All three build scripts pick the tag up from `git describe --tags` automatically. The final artefact lands in `dist/Tlamatini_Release_v1.10.0/`, named for the version so the file you hand to a user is unambiguous before they even unzip it.
...
- **Release folder name** — `dist/Tlamatini_Release_v1.10.0/`.
- **Runtime surfaces** — the About dialog renders `Tlamatini v{{ version }}` (Django context processor); the startup banner prints `--- [VERSION] Tlamatini 1.10.0` to both the console and `tlamatini.log`; `GET /agent/version/` returns `{"version":"1.10.0","commit":"abc1234","date":"…","source":"generated"}` as an **open** endpoint suitable for a health-check.
...
| **Notifier** | LangGraph-based notification agent — in-browser popup + native Windows toast (Action Center). |
...
- **Notifier Grows a Native Windows Toast — a Second, OS-Level Notification Surface — 2026-05-29** — The **Notifier** agent now fires **two** notifications on a match instead of one. The original in-browser popup (it drops a `notification.json` the chat UI polls and renders as an in-page bubble, with optional `.wav`) stays exactly as it was; *additive* to it, Notifier now also raises a real **Windows toast** — the OS banner that slides in bottom-right and persists in the Action Center, exactly like a Chrome or YouTube notification — carrying the Tlamatini logo, and clicking it focuses the **existing** Tlamatini browser tab (via the `tlamatini:` URL protocol + a Win32 focus helper) rather than opening anything new. The toast lives in a new module `agent/native_toast.py`, mirrored inline inside `agent/agents/notifier/notifier.py` (`trigger_native_toast`) because pool subprocesses can't import `agent.*`. Three hard contracts make it actually work on Windows: (1) it shells out to **Windows PowerShell 5.1 (`powershell.exe`), never `pwsh`** — PowerShell 7 can't load the WinRT toast type projections; (2) it uses the AUMID **`XAIHT.Tlamatini.Server`** (the same identity `manage.py` already sets for the taskbar), *registered* under `HKCU\Software\Classes\AppUserModelId\…` so the toast shows the name + icon and persists in the Action Center; (3) every registry write is **HKCU / non-admin only** — an elevated process gets its toasts suppressed and the cross-integrity focus dance fails. Identity + protocol are registered once at Django startup (`apps.py` → `native_toast.register_all()`). New `notifier/config.yaml` keys under `target`: `native_toast` (default true), `toast_title`, `toast_image` (empty → the registered HKCU IconUri), `toast_click` (`focus`/`none`); it reuses `sound_enabled`. `build.py` ships `agent/static/agent/img/Tlamatini.png` for the toast logo. Coverage: 25 tests in `agent/test_native_toast.py` — but because this is OS integration where green mocks can lie, each contract was also **proven live on a dev box** (banner shows, icon shows, click focuses the existing tab). Agent / tool / skill counts unchanged — this is a second surface on an existing agent.
