...
> **The Book of Tlamatini** — a step-by-step guide to running, using, and mastering a locally-deployed AI developer assistant with RAG, Multi-Turn tool orchestration, ACPX external-CLI delegation, an Unreal MCP client for driving Unreal Engine 5 from chat or canvas, a visual workflow designer, 70 drag-and-drop agent types, and a backend Flow Compiler that turns the live canvas — or a chat-generated tool-call log — into a registry-validated, secret-redacted, source-and-frozen-portable workflow.
>
...
- **Part IV — The Tlamatini Bestiary**: compact one-row-per-agent reference for all 70 workflow agents.
...
4. **A visual workflow designer** where you drag 70 different agent types onto a canvas (including the microcontroller-firmware trio **STM32er** / **ESP32er** / **Arduiner** and the **Unrealer** for driving Unreal Engine 5 — see bonus chapter §57), wire them up, and run the result as an unattended `.flw` workflow. Save, Validate, and Start all funnel the canvas through a backend **Flow Compiler** (`agent/services/flow_compiler.py`) that consults a single Agent Contract registry — so a flow that runs in source mode runs identically in a frozen `.exe` install.
...
28 agents emit Parametrizer-compatible sections:

Apirer, Gitter, Kuberneter, Crawler, Summarizer, File-Interpreter, Image-Interpreter, File-Extractor, Prompter, FlowCreator, Kyber-KeyGen, Kyber-Cipher, Kyber-DeCipher, Gatewayer, Gateway-Relayer, Googler, **Playwrighter**, **ACPXer**, Shoter, Mouser, **Windower**, **Unrealer**, **Reviewer**, **Analyzer**, **Kalier**, **STM32er**, **ESP32er**, **Arduiner**.
...
A compact reference for all 70 workflow-agent types. Spotlight chapters for **Parametrizer** (§25) and **Gatewayer** (§26) above; **Unrealer** gets a full bonus chapter at §57.
...
## 28. Wrapped chat-agent tools (45)
...
git tag -a v1.13.0 -m "Release 1.13.0: <one-liner>"   # annotated tag
...
All three build scripts pick the tag up from `git describe --tags` automatically. The final artefact lands in `dist/Tlamatini_Release_v1.13.0/`, named for the version so the file you hand to a user is unambiguous before they even unzip it.
...
- **Release folder name** — `dist/Tlamatini_Release_v1.13.0/`.
- **Runtime surfaces** — the About dialog renders `Tlamatini v{{ version }}` (Django context processor); the startup banner prints `--- [VERSION] Tlamatini 1.13.0` to both the console and `tlamatini.log`; `GET /agent/version/` returns `{"version":"1.13.0","commit":"abc1234","date":"…","source":"generated"}` as an **open** endpoint suitable for a health-check.
