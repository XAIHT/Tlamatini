...
git tag -a v1.49.1 -m "Release 1.49.1: <one-liner>"   # annotated tag
...
All three build scripts pick the tag up from `git describe --tags` automatically. The final artefact lands in `dist/Tlamatini_Release_v1.49.1/`, named for the version so the file you hand to a user is unambiguous before they even unzip it. The current `v1.49.1` tag remains reachable from `HEAD`, so the bare runtime version stays `1.49.1` even though the worktree is one commit beyond the tagged commit.
...
- **Release folder name** — `dist/Tlamatini_Release_v1.49.1/`.
- **Runtime surfaces** — the About dialog renders `Tlamatini v{{ version }}` (Django context processor); after the release tag/build, the startup banner prints `--- [VERSION] Tlamatini 1.49.1` to both the console and `tlamatini.log`; `GET /agent/version/` returns `{"version":"1.49.1","commit":"abc1234","date":"…","source":"generated"}` as an **open** endpoint suitable for a health-check.
