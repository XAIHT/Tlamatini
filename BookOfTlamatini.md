...
| **PPTXer** | DECK COMPOSER — the third member of the document family (File-Extractor / File-Interpreter READ documents, PDFer *composes* a PDF, LaTeXer *typesets* one, PPTXer *builds the slides*). Turns Tlamatini's own answer, Markdown, a plain outline or a JSON deck spec into ONE finished `.pptx` carrying embedded images (including images **FETCHED FROM THE INTERNET** by URL), video, audio, native editable diagrams, real tables and generated artwork. `action` ∈ `create` (DEFAULT) / `outline` (the slide PLAN only, nothing written) / `render` / `audit` / `info` / `fonts` / `validate`. **ZERO new dependencies** — python-pptx + Pillow already ship, and every backend loads lazily so a missing one degrades to `engine_unavailable` instead of crashing. Built above all for **MARKETING and GAMING**: 24 nuances, EIGHT of them gaming (`esports_tournament`, `cyberpunk_tech`, `retro_arcade`, `fantasy_lore`, `military_tactical`, `streamer_kit`, `game_design_doc`, `game_trailer_beat`) and EIGHT marketing, with English and Spanish aliases, plus `predominant_color` deriving a whole 50-role palette in **OKLab** (a red→blue midpoint keeps L=0.54 there against 0.42 in sRGB, which is why the gradients stay luminous instead of going muddy through grey). Every typeface installed on the machine is reachable through 15 pairings — Terminator, Nasalization, Verdana, Times New Roman, Courier New. **⚠️ IT LOOKS AT THE SLIDES IT WROTE.** python-pptx performs NO layout, so a successful save proves only that the XML is well-formed and says nothing about whether the text fits: every element is placed from **REAL font metrics** (a half-em estimate is **+78%** wrong on `lllllllllll` and **−38%** wrong on `WWWWWWWWWWW`), so overlap is prevented BY CONSTRUCTION rather than detected afterwards — and the finished deck is then RENDERED, through the **INSTALLED POWERPOINT via COM** when there is one, and those pixels MEASURED for overlap, off-slide ink, overflowing text frames and true contrast. `ground_truth` is True only when PowerPoint's own renderer produced them, and a deck with measured defects reports `created_with_findings` and never claims to be clean. Projected-slide contrast floors are stricter than a page's (7.0 body, 4.5 display, ×1.06 safety) because a slide is read from the back of a room on an uncalibrated projector, and **the artwork yields to legibility, never the reverse**: the ornament is faded until every text role clears its own floor, so the palette is never touched. Media is identified by its **BYTES** (45 magic signatures — a server's Content-Type header is not evidence) and the SSRF guard fails **CLOSED**, re-validating every redirect hop. ⚠️ PowerPoint COM must be torn down completely — `Quit()` alone leaks a POWERPNT.EXE, so the teardown drops every reference, `gc.collect()`s, `CoUninitialize()`s and reaps only strays, never a PID that existed BEFORE the render. Saves to `Documents/TlamatiniPPTX` with a collision-proof name; a fail-safe preflight REFUSES rather than write an empty or wrong deck, and `target_agents` fire on success, failure AND refusal so a Forker can branch on `{status}` / `{layout_clean}` / `{ground_truth}`. State-changing → in the Exec Report and on the Ask-Execs tier-A allowlist. Emits `INI_SECTION_PPTXER<<<` (`action`, `status`, `output_path`, `slide_count`, `shape_count`, `tables`, `generated_art`, `nuance`, `palette`, `font_display`, `render_tier`, `ground_truth`, `layout_clean`, `overlaps`, `text_overflows`, `bytes`). Canvas counterpart of the `chat_agent_pptxer` Multi-Turn tool. |
...
| **PPTXer** | Deck composer — the third member of the document family (PDFer *composes* a PDF, LaTeXer *typesets* one, PPTXer *builds the slides*). Turns an answer, some Markdown or a plain outline into a finished PowerPoint file with embedded pictures, video, audio, diagrams and real tables, dressed in one of twenty-four treatments chosen from the content itself and aimed above all at marketing and gaming. Unusually, it then **looks at its own work**: the library that writes `.pptx` files does no layout at all, so a clean save proves nothing — PPTXer measures every word in the real typeface before placing it, opens the finished deck in the installed PowerPoint, exports each slide and measures those pixels for overlapping text, text outside its box and poor contrast, and says so plainly when it finds any. The 89th entry in the agent catalog. |
