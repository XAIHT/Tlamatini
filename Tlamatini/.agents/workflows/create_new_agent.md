...
- **⚠️ CREATING A NEW SECTION COSTS TWO PROMPTS, NOT ONE (2026-09-12, learned on VOICE COMMANDS).** A new section needs (a) a `('<key>', '<Label>')` entry in `views.PROMPT_CATEGORY_ORDER` — position it deliberately; `voice_commands` sits at **index 0**, ahead of Getting Started, because Angela asked for it there — and (b) a **Step-by-Step opener at `sort_rank` 10**. That opener is not optional: `test_known_section_openers` fails for any live section absent from its `expected_first` map, and `test_every_section_opens_with_a_genuine_step_by_step_wizard` reads the opener's CONTENT for the Step-by-Step checkbox AND a promise to WAIT. Ship a section with one card and both go RED. Add your section to `expected_first` in the same pass. **Nothing else changes** — the catalog modal is fully data-driven from `/agent/list_prompts/`, so a new section needs no JS, no CSS, no template and no `STATIC_VERSION` bump. Historical migrations that mirror the category order (e.g. `0179`'s `_CATEGORY_ORDER`) are frozen history — do NOT edit them.
- **⚠️ THE MODE BADGES ARE DERIVED FROM YOUR PROMPT TEXT — you cannot set them.** `tools_dialog.js::classifyPromptModes` decides, and clicking the card then ticks exactly those toolbar boxes. It first SCRUBS every `do not use …` clause (so a tool your prompt FORBIDS is never counted as one it requires), then: an `acp_*` / `invoke_skill` / `list_skills` token ⇒ **ACPX** (which implies Multi-turn); a `chat_agent_*` mention or the words "multi-turn" ⇒ **Multi-turn**; the **hyphenated** `step-by-step` token near "tick/check/enable", or followed by wizard/checkbox/mode/… ⇒ **Step-by-Step**; and every non-one-shot card also gets **Exec-report**. Consequences when authoring: to make a card ACPX you must NAME an ACPX tool in a non-forbidding sentence (parking it inside "do not use …" silently deletes the badge); to keep a card OFF Step-by-Step, never write the hyphenated token (numbered "Step 1 / Step 2" is safe); and a PRE-FLIGHT line naming the checkboxes is how you document what the badges will be. Worked example: `agent/migrations/0201_add_voice_commands_section.py`, pinned by `agent/test_voice_commands_catalog.py`.
