...
class RecorderHelperTests(unittest.TestCase):
    ...
    def test_get_music_dir_nonempty(self):
        path = self.mod.get_music_dir()
        self.assertTrue(path)
        self.assertIsInstance(path, str)

    def test_resolve_output_dir_default_is_tlamatini_records(self):
        out = self.mod.resolve_output_dir({})
        self.assertTrue(out.endswith(os.path.join('Music', 'TlamatiniRecords'))
                        or out.endswith('TlamatiniRecords'))
...
class RecorderRegistryTests(SimpleTestCase):
    ...
    def test_not_in_exec_report(self):
        # Recorder is observational (like Shoter / Camcorder) -> NOT a
        # state-changing Exec-Report row.
        mcp_path = os.path.join(_REPO_AGENT_DIR, 'mcp_agent.py')
        with open(mcp_path, 'r', encoding='utf-8') as handle:
            text = handle.read()
        self.assertNotIn('chat_agent_recorder', text)
