...
WRAPPED_CHAT_AGENT_SPECS: tuple[ChatWrappedAgentSpec, ...] = (
    ...
    ChatWrappedAgentSpec(
        ...
        purpose="Send OR receive Telegram messages via the OFFICIAL Telegram BOT API (a bot_token from @BotFather — NOT Telethon/api_id/api_hash/phone-login). Set mode='send' to SEND a message (then it starts its downstream target_agents and exits), or mode='receive' to WAIT for incoming messages for rx_max_seconds then start target_agents and exit (mode='auto' picks send when a message/recipient is given, else receive). Address the recipient EITHER by contact_name (a person's name resolved from the Contacts book, contacts.json — PREFERRED when the user names someone) OR by telegram.chat_id (@username / numeric id). Required cred: telegram.bot_token; for sending also message + a recipient (contact_name or telegram.chat_id); for receiving, optional rx_from_chat_id / rx_match filters and rx_max_seconds.",
        example_request="Send Telegram with mode='send' and telegram.bot_token='<BotFather token>' and contact_name='Ana Ricardo Lazcano' and message='Im ok!'  (or use telegram.chat_id='@ana_lazcano' instead of contact_name; to receive use mode='receive' and rx_max_seconds=30)",
