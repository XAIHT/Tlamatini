...
WRAPPED_CHAT_AGENT_SPECS: tuple[ChatWrappedAgentSpec, ...] = (
    ...
    ChatWrappedAgentSpec(
        ...
        purpose=(
            ...
            "stops on its own after 3.5 seconds of silence. "
            ...
            "silence_timeout_seconds=N to widen or shorten the silence window (default 3.5), and "
            "device_index=N or "
        ...
        example_request=(
            ...
            "model='base' and NO record_seconds at all (so it listens until 3.5s of silence), OR "
    ...
    ChatWrappedAgentSpec(
        ...
        example_request="Send Telegram with mode='send', provider='me' (send as my own account) and contact_name='<REDACTED>' and message='Im ok!'  -- or provider='bot' to send AS the bot instead; omit provider to auto-pick. (contacts.json may keep telegram='@ana_lazcano'; sending as the bot cannot cold-DM a private @username unless they pressed Start, so 'as me' is best for private people; to receive use mode='receive' and rx_max_seconds=30)",
    ...
    ChatWrappedAgentSpec(
        ...
        example_request="Send WhatsApp with mode='send', provider='me' (from my OWN number via WhatsApp Web) and contact_name='<REDACTED>' and message='Im ok!'  -- or provider='cloud' to send from the business number via the official API; omit provider for the cloud default. (web mode needs a one-time QR scan and ignores templates; cloud cold messages need template='hello_world' + template_language='en_US'; the `to` field takes a raw +E.164 number instead of contact_name; mode='receive' with rx_max_seconds=30 receives instead of sending)",
    ...
    ChatWrappedAgentSpec(
        ...
        purpose=(
            ...
            "lines [default], 'files' = matching paths only, 'count' = per-file match counts, "
            "'lines' = READ ONE FILE VERBATIM), and max_results (cap, default 200). Read-only: it "
            "never changes any file. Noise dirs (.git, node_modules, venv, __pycache__, dist, "
            "build, ...) are pruned and binary or unreadable files are skipped automatically. "
            "IMPORTANT - output_mode='lines' is how you READ a file: it takes NO pattern and "
            "returns the exact text of one file, so use it INSTEAD of an execute_command "
            "type/cat/sed when you need to see a file's contents. Give it start_line and end_line "
            "(1-based and INCLUSIVE; 0 means from the first line / to the last) plus line_numbers: "
            "line_numbers=false returns the exact text AND an extra content_b64 field holding "
            "base64 of the exact bytes - DECODE content_b64 when you need an old_string for "
            "chat_agent_editor, because the agent log rewrites line endings on Windows so the "
            "plain body is not reliable character-for-character. line_numbers=true (default) "
            "prefixes each line 'N: ' for navigation and emits no content_b64. A directory is "
            "refused - use Globber to pick the file first. Emits INI_SECTION_GREPPER (pattern, "
            "path, glob, matches, files_searched, truncated, start_line, end_line, "
            "lines_returned, total_lines, content_b64, status "
            "matches/no_matches/listed/refused/not_found/error) with the results - or the verbatim "
            "slice - as the body. Use this to FIND where something appears in code or text, then "
            "again in 'lines' mode to READ that region back before editing it."
        ),
        example_request=(
            "Run Grepper with pattern='TODO', path='C:/proj/app', glob='*.py', output_mode='content' "
            "-- then, to read the exact bytes of a region for an Editor old_string: Run Grepper with "
            "path='C:/proj/app/main.py', output_mode='lines', start_line=40, end_line=60, "
            "line_numbers=false"
        ),
    ...
    ChatWrappedAgentSpec(
        ...
        purpose=(
            ...
            "  - styles   -> list visual styles and aliases (writes nothing)\n"
            ...
            "Visual style and content type are separate choices:\n"
            "  - style='...' -> select a visual identity: kawaii_cloud (cute), baby_dream "
            "(baby), baby_peach, candy_pop, storybook_garden, pastel_sorbet; cyberpunk, "
            "neon_noir, holographic, synthwave; cosmic_nebula, astral_atlas, lunar_minimal, "
            "solar_flare, aurora; circuit_board (electronics), blueprint, quantum_chip, "
            "oscilloscope; aztec_obsidian, quetzal_jade, tlamatini_celestial (tlamatini), "
            "xeno_codex (alien), alien_biolume. style=auto preserves the automatic theme. "
            "These change palette, type, spacing and original vector cover art. "
            "Content nuance still controls the decoration ceiling: marketing/science "
            "permit rich artwork, formal or uncertain content remains restrained. "
            "Honor the user's visual preference without changing the document's meaning.\n"
            ...
            "page_count, bytes, images_used, engine, nuance, style, style_family, nuance_confidence, "
            ...
            "With layout_audit=true, the atelier path reopens the finished PDF and checks "
            "overlap, off-sheet content, blank pages and contrast. status='created' alone "
            "confirms file creation, not a clean audit. Inspect layout_clean and response_body; "
            "a blank audit field is unverified, zero overlaps does not rule out other findings, "
            "and repairs lists solver actions rather than certifying success. Custom css with "
            "engine='auto' selects legacy rendering; styles do not restyle legacy, image-only "
            "or merged original pages. mode='styles' returns status='inspected' and a catalog, "
            "not a PDF. Unknown visual styles fall back to the semantic design with a diagnostic. "
            "PDFer has no success contract field. Optional ollama_design is off by default. "
    ...
    ChatWrappedAgentSpec(
        ...
        purpose=(
            ...
            "content, detects a treatment or uses one of 36 named styles, designs the whole visual "
            "system, and measures text and geometry before checking the saved deck. "
            "Dense cards, tables and columns continue across slides to retain complete text; "
            "256 characters is a stress-case length, not a content cap. There are 17 font pairings "
            "resolved against installed fonts. Audit findings and coverage determine what was "
            "verified; creation alone does not prove every slide is free of defects.\n\n"
            ...
            "legal_compliance, safety_briefing. Additional visual styles: "
            "swiss_editorial, warm_editorial, midnight_luxe, botanical, oceanic, "
            "blueprint, terracotta, nordic_frost, bauhaus, lavender_studio, "
            "monochrome_ink, sunset_coral. Each is selected with `nuance`.\n\n"
            ...
            "RENDERING: render='auto' tries PowerPoint, LibreOffice and then an approximate "
            "Pillow preview. Native applications are optional for creation. render='none' skips "
            "slide images while layout_audit=true still measures geometry. A bounded worker "
            "may clean up only Office processes attributable to that worker.\n\n"
            "RESULT: read status, success, output_path, layout_clean, ground_truth, render_tier, "
            "slides_rendered and slide_count together with response_body confidence, skipped "
            "measurements and audit errors. Header ground_truth identifies native rendering; "
            "it does not itself prove complete audit coverage. Missing audit evidence is not a "
            "pass. status='created_with_findings' can still carry success=true: report findings "
            "and incomplete measurements instead of claiming a clean deck. Report required media "
            "that could not be embedded. outline/info/fonts return listed and validate returns "
            "validated; these create no new deck. Existing-deck render/audit return ok or findings. "
            "Styles use nuance; there is no separate style parameter or style-catalog action."
        ),
        example_request=(
    ...
    ChatWrappedAgentSpec(
        ...
        purpose=(
            ...
            "  - list_styles          -> JSON catalogue of 30 visual identities; no TeX engine needed\n"
            ...
            "SIGNATURE STYLES: choose style independently of template. Editorial: scholarly, "
            "swiss, atelier. Cute/nursery: cute_candy, baby_blush, baby_sky, storybook, lullaby. "
            "Neon: cyberpunk, neon_noir, holographic, synthwave. Astronomy: cosmic_nebula, "
            "astral_atlas, lunar_minimal, solar_flare, aurora, deep_space. Electronics: "
            "circuit_board, blueprint, quantum_chip, oscilloscope. Beyond-planet Tlamatini: "
            "aztec_obsidian, quetzal_jade, tlamatini_celestial, xeno_codex, alien_biolume, "
            "solar_codex, obsidian_portal, quetzal_supernova. Use action='list_styles' for aliases. "
            "Set subtitle, style_mode='screen|print', style_decoration='none|restrained|rich', "
            "style_cover=true|false, predominant_color='#RRGGBB'. Styles create portable TikZ "
            "art, coordinated headings, page furniture and contrast-corrected colors. "
            "Use scaffold_compile + template + style + content_b64 for a styled PDF in one call; "
            "create_file and bare-fragment compile also support style. Complete .tex files and "
            "projects own their preamble: omit style when compiling those. Empty style retains "
            "legacy output. Styled metadata remains LaTeX (escape literal &, %, _, #). "
            "Styled bodies can use TLcallout, listings, booktabs/tabularx, TLtablehead and "
            "TLcellhead (wrap EVERY header cell). Results include style, style_family and style_mode; "
            "list_styles returns listed, success=true, style_count=30, distribution=not_probed "
            "and a JSON catalog, not a PDF. Unknown styles refuse. Beamer scaffolds take frame "
            "body content, not a sequence of complete frames; for explicit frames use complete "
            "source without a style override. style_cover=false retains Beamer's title frame "
            "while omitting its illustration. find_text_b64/replace_text_b64 preserve edits; "
            "successfully decoded fields take precedence over their literal counterparts.\n\n"
            "REQUIREMENT — an installed TeX distribution for compilation; Tlamatini does not "
            "bundle one. LaTeXer supports MiKTeX with on-demand package installation, and detects "
            "TeX Live/MacTeX too; its acquire repair rung can use an available tlmgr. Catalog and "
            "source-authoring actions need no compiler. If none is present, compilation refuses "
            ...
            "errors, warnings, success and status (compiled | compiled_with_errors | degraded | "
            "created | edited | read | listed | validated | invalid | analyzed | cleaned | refused | "
            "not_found | not_unique | engine_unavailable | installer_launched | error). status="
            "'compiled_with_errors' means a PDF EXISTS but LaTeX reported errors — say so, "
            "and quote the errors instead of calling it a success. status='degraded' means blocks "
            "were removed to rescue a partial PDF; both outcomes return success=false. "
            "A clean compiled result (success=true, errors=0) is the stop condition: deliver "
            "output_path without further edits unless a remaining requirement warrants them. "
            "Compilation/style metadata does not certify visual layout; inspect overflow warnings "
            "and requested visual evidence separately. LaTeXer has no normal layout_clean field.\n\n"
            "REPAIR: repair=true enables lint, preamble, rules, log_directed, acquire, engine_swap, "
            "model, bisect; repair_rungs=[] enables all. repair_write_back=false preserves the "
            "author's source and builds a .latexer-fixed.tex copy. The model rung is configured "
            "by default; repair_model='' disables it. repair_model_timeout defaults to 600s, "
            "repair_bisect_max_probes to 14, and command_timeout=600 bounds individual external "
            "invocations, not the whole action. Failed model rewrites are discarded; a timeout "
            "or unreachable model prevents destructive bisect. Read the repair report in "
            "response_body; never present removed content as a complete deliverable.\n\n"
        ...
        example_request=(
            ...
            "input_text_b64='<base64 of the whole .tex>', filename='doc.pdf'   ||   STYLED PDF: "
            "Run LaTeXer with action='scaffold_compile', template='article', "
            "style='tlamatini_celestial', title='Knowledge beyond the stars', "
            "content_b64='<base64 of the LaTeX body>', filename='celestial.pdf'"
        ),
        aliases=(
