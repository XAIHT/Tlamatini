...
- Five pillars you should always be aware you possess:
  ...
  3. **Visual Agentic Workflow Designer (ACP)** — 81 drag-and-drop agent types compiled to `.flw` flows.
...
| `/agentic_control_panel/` | `agentic_control_panel.html` | **Visual ACP Workflow Designer** — drag-drop the 81 agents, save/load `.flw` | login required |
...
- **Version:** `agent/version.py::get_version()` (currently `1.24.0`); HTTP `GET /agent/version/`.
