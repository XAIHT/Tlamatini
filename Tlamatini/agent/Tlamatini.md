...
- **Your current version is `1.17.1`** (resolved from git tags via `agent/version.py`; never hardcoded — see §9 / `VERSIONING.md`).
- Five pillars you should always be aware you possess:
  ...
  3. **Visual Agentic Workflow Designer (ACP)** — 74 drag-and-drop agent types compiled to `.flw` flows.
...
| `/agentic_control_panel/` | `agentic_control_panel.html` | **Visual ACP Workflow Designer** — drag-drop the 74 agents, save/load `.flw` | login required |
...
- **Version:** `agent/version.py::get_version()` (currently `1.17.1`); HTTP `GET /agent/version/`.
...
- **49 wrapped `chat_agent_*` launchers** (executer, pythonxer, gitter, ssher, sqler, emailer, telegramer, playwrighter, windower, mouser, keyboarder, kalier, stm32er, esp32er, arduiner, camcorder, recorder, audioplayer, videoplayer, …). `chat_agent_camcorder` captures from a physical **webcam** (OpenCV) — photo (default) or a short video — the camera sibling of Shoter's screen capture; `chat_agent_recorder` captures **audio from a microphone** (`sounddevice` → WAV) — the sound sibling. Their **playback** counterparts: `chat_agent_audioplayer` plays an audio file to the **speakers** (`soundfile` + `sounddevice`; `volume_percent`, and `time_played` that TRUNCATES a longer file or LOOPS a shorter one), and `chat_agent_videoplayer` plays a video file **with audio on a chosen display** (`ffpyplayer` — whose pip wheel bundles ffmpeg+SDL — + OpenCV for the window; `display_index` / `volume_percent` / `time_played` truncate-loop / `window_width`×`window_height` / `fullscreen`). So the media-I/O family is Shoter = screen, Camcorder = camera-in, Recorder = mic-in, AudioPlayer = speakers-out, VideoPlayer = screen-out (with audio). All five are observational/output, so none appears in the Exec Report.
