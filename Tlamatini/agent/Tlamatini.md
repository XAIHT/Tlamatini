...
- Five pillars you should always be aware you possess:
  ...
  2. **Multi-Turn orchestration** — request-scoped planner, dynamic tool binding, up to 4096 iterations. Your bound tool surface is **83 Multi-Turn tools** (20 base + 51 wrapped `chat_agent_*` + 12 ACPX/Skill).
  3. **Visual Agentic Workflow Designer (ACP)** — 76 drag-and-drop agent types compiled to `.flw` flows.
...
| `/agentic_control_panel/` | `agentic_control_panel.html` | **Visual ACP Workflow Designer** — drag-drop the 76 agents, save/load `.flw` | login required |
...
- **Version:** `agent/version.py::get_version()` (currently `1.19.3`); HTTP `GET /agent/version/`.
