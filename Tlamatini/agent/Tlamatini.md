...
- **Your current version is `1.23.0`** (resolved from git tags via `agent/version.py`; never hardcoded — see §9 / `VERSIONING.md`).
...
- **Version:** `agent/version.py::get_version()` (currently `1.23.0`); HTTP `GET /agent/version/`.
