...
def update_pptxer_connection_view(request, agent_name):
    """
    Update a PPTXer agent's config.yaml when connections are made/removed.

    PPTXer is an active (source-side) agent like PDFer/LaTeXer: drawing a wire
    FROM a PPTXer node TO another agent records that downstream agent in the
    PPTXer's ``target_agents`` (started after the deck is built — success,
    failure OR a fail-safe preflight refusal, so a downstream Forker can always
    branch on ``{status}`` / ``{layout_clean}``). Incoming wires are written by
    the UPSTREAM agent's own connection view, so PPTXer only ever needs the
    target-side path.
    ...
    try:
        ...
        # Parse agent_name to pool folder name: 'pptxer-1' -> 'pptxer_1'
        parts = agent_name.split('-')
        cardinal = None
        if parts[-1].isdigit():
            cardinal = parts.pop()
        ...
        if not os.path.exists(config_path):
            return HttpResponse(json.dumps({
                "success": False,
                "message": f"PPTXer config not found: {config_path}"
            }), content_type='application/json', status=404)
        ...
        print(f"Error updating PPTXer connection: {e}")
        return HttpResponse(json.dumps({"error": str(e)}), content_type='application/json', status=500)
...
def get_parametrizer_dialog_data_view(request, agent_name):
    ...
    try:
        ...
        if source_base is None:
            return HttpResponse(json.dumps({
                "success": False,
                "message": f"Source agent '{source_agent}' does not produce structured output, so "
                           "it cannot be a Parametrizer source. Only agents that emit an "
                           f"INI_SECTION_<TYPE> block are allowed ({len(PARAMETRIZER_ALLOWED_SOURCES)} "
                           "structured-output agents are currently registered, e.g. Apirer, "
...
def external_mcps_remove_view(request):
    ...
    try:
        ...
        if not isinstance(keys, list) or not keys:
            return JsonResponse(
                {"ok": False, "error": "keys must be a non-empty list"},
                status=400,
            )
