...
def get_pictures_dir() -> str:
    """
    Resolve the current user's *Pictures* known-folder as a real absolute path.

    On a localized Windows the folder is *displayed* as e.g. "Imagenes" /
    "Imágenes", but the on-disk path is what we must write to. The Win32
    known-folder API (SHGetKnownFolderPath, FOLDERID_Pictures) returns the
    true path regardless of the display name. We fall back to ~/Pictures on
    any failure or non-Windows host.
    """
    if os.name == 'nt':
        try:
            import ctypes
            ...
            # FOLDERID_Pictures = {33E28130-4E1E-4676-835A-98395C3BC476}
            folderid_pictures = _GUID(
                0x33E28130, 0x4E1E, 0x4676,
                (ctypes.c_byte * 8)(0x83, 0x5A, 0x98, 0x39, 0x5C, 0x3B, 0xC4, 0x76),
            )

            path_ptr = ctypes.c_wchar_p()
            res = ctypes.windll.shell32.SHGetKnownFolderPath(
                ctypes.byref(folderid_pictures), 0, None, ctypes.byref(path_ptr)
            )
            ...
            logging.warning(f"⚠️ Could not resolve Pictures known-folder via Win32 API: {e}")
...
def resolve_output_dir(config: Dict) -> str:
    ...
    - Otherwise default to <Pictures>/TlamatiniCamcorder.
    """
    ...
    if configured:
        ...
        return configured
    return os.path.join(get_pictures_dir(), 'TlamatiniCamcorder')
