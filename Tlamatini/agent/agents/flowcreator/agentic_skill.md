...
- **Application example**: Starter → Crawler (gather the launch material) → Parametrizer (map `{response_body}` into PPTXer's `input_text`) → PPTXer (`nuance: esports_tournament`, `predominant_color: "#12E2A3"`) → Parametrizer (map `{output_path}` into Emailer's attachment) → Emailer → Ender. A second common shape is Starter → Shoter → Parametrizer (map `{output_path}` into PPTXer's `images`) → PPTXer → Ender.
