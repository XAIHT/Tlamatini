...
### 60. Unrealer
- **Purpose**: Drives an Unreal Engine 5 editor instance via the Unreal MCP plugin's TCP socket protocol (default 127.0.0.1:55557 — the plugin must already be running inside UE5). Sends one JSON command per execution (`{"type": <command>, "params": {...}}`) and captures the full Unreal response into an `INI_SECTION_UNREALER` block so Parametrizer can route specific fields into downstream agents. Triggers `target_agents` on success OR error so the flow can branch on the section's `status` / `error` fields.
- **Used for**: Unattended manipulation of an Unreal Engine 5 project. Supports the complete 28-command Unreal MCP surface: actor manipulation (`get_actors_in_level`, `find_actors_by_name`, `spawn_actor`, `delete_actor`, `set_actor_transform`, `get_actor_properties`, `set_actor_property`, `spawn_blueprint_actor`), Blueprint creation and editing (`create_blueprint`, `add_component_to_blueprint`, `set_static_mesh_properties`, `set_component_property`, `set_physics_properties`, `compile_blueprint`, `set_blueprint_property`), Blueprint node-graph wiring (`add_blueprint_event_node`, `add_blueprint_input_action_node`, `add_blueprint_function_node`, `connect_blueprint_nodes`, `add_blueprint_variable`, `add_blueprint_get_self_component_reference`, `add_blueprint_self_reference`), input mappings (`create_input_mapping`), and UMG widget building (`create_umg_widget_blueprint`, `add_text_block_to_widget`, `add_button_to_widget`, `bind_widget_event`, `add_widget_to_viewport`, `set_text_block_binding`).
- **Aimed at**: Building visual game-dev workflows in Tlamatini that talk to a running UE5 editor — automated level setup, Blueprint scaffolding, batch actor spawning, widget UI assembly, CI-style Blueprint compilation, or "watch a log → take action in the editor" loops via Raiser. The visual-canvas counterpart of the wrapped `chat_agent_unrealer` Multi-Turn tool.
- **Application example**: A Croner triggers a nightly build at 02:00. A first Unrealer runs `create_blueprint` to scaffold a new Blueprint class. A Parametrizer copies the resulting blueprint name into a second Unrealer that runs `add_component_to_blueprint` with a `StaticMeshComponent`. A third Unrealer runs `compile_blueprint`. A fourth Unrealer runs `spawn_blueprint_actor` at `[0, 0, 100]`. A final Notifier signals completion.
...
- **Config parameters**:
  ...
  - `command`: "get_actors_in_level" (one of the 28 supported Unreal MCP commands)
