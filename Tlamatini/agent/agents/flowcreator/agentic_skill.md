...
| **latexer** | Typesets LaTeX with mathematics, citations, 30 explicit styles and an eight-rung repair ladder. Compilation needs an installed TeX distribution; list_styles needs none. Route degraded/error results before delivery. | YES | Action |
...
### 75. Whisperer
- **Purpose**: SPEECH-TO-TEXT (STT / voice recognition): turns SPOKEN AUDIO into a STRING of text. 100% self-sufficient for the microphone — it OPENS, CONFIGURES (channels, sample rate, gain) and RECORDS the mic ITSELF (does NOT depend on Recorder), or transcribes a given audio FILE. **⚠️ SINCE 2026-09-11 THE DEFAULT IS A SOUND GATE, NOT A FIXED BLOCK**: with `record_seconds: 0` (the default) it keeps recording while the speaker is still talking and STOPS ON ITS OWN after `silence_timeout_seconds` (default 3.5) of continuous silence, up to the `max_record_seconds` ceiling (default 300). Give `record_seconds` a number and it records exactly that long with the gate OFF — the same idiom as `sample_rate: 0` meaning the native rate. Runs faster-whisper LOCALLY by default (auto-detects an NVIDIA GPU and ALWAYS falls back to CPU), or a cloud Whisper API (Groq/OpenAI); optionally tidies the transcript with an Ollama LLM (Ollama CANNOT transcribe — post-processing only). Emits an `INI_SECTION_WHISPERER` block (`transcript_path`, `audio_path`, `input_source`, `engine`, `model`, `device`, `language`, `duration_seconds` — the length ACTUALLY captured, not the length requested —, `segments`, `word_count`, `status`, plus the appended gate fields `capture_mode` / `stop_reason` / `silence_timeout_seconds` / `speech_seconds`, plus the transcript text as `response_body`) and ALWAYS triggers `target_agents`. The speech-to-text sibling of Talker (text-to-speech); read-only/observational, but its wrapped Multi-Turn tool IS captured in the Exec Report like every other `chat_agent_*` (2026-06-07 completeness contract).
...
- **Aimed at**: STT steps. `input_source` mic (DEFAULT — records the mic itself) or file (set `audio_file`). **Leave `record_seconds` at 0 for dictation** (the flow waits for the speaker to finish); set it only when the flow genuinely needs a fixed-length clip. `silence_timeout_seconds` widens/shortens the silence window, `silence_gate` (auto/on/off) forces the mode, `max_record_seconds` caps a gate that never fires, and `silence_threshold_db` (0 = measure the room automatically) retunes a noisy room. `engine` faster-whisper (default; `model` tiny/base/small/medium/large-v3/large-v3-turbo, `device` auto/cuda/cpu with guaranteed CPU fallback) or cloud-groq/cloud-openai (needs an API key). `language` "" auto-detects; `task: translate` outputs English. Pair with Parametrizer to carry the `{response_body}` transcript into a downstream agent, or a Forker to branch on `{status}` (transcribed/empty/engine_unavailable/error) or on `{stop_reason}` (silence/duration/max_duration/fixed_fallback).
- **Application example**: Starter → Whisperer (dictation: pass NO `record_seconds`, so it stops after 3.5 s of silence) → Parametrizer (map Whisperer's `{response_body}` into a Prompter `{question}`) → Prompter → Ender. Or a full voice loop: Starter → Whisperer → Prompter → Talker → Ender (listen → think → speak). ⚠️ In a LOOPING voice flow give the gate a ceiling you are happy to wait for (`max_record_seconds`), because an empty room only ends the turn after the silence window elapses.
- **⚠️ THE VOICE-COMMAND PATTERN (2026-09-12) — the transcript IS the instruction.** The chat surface got a whole catalog section for this (**VOICE COMMANDS**, cards #121/#122), and the canvas equivalent is a Parametrizer that copies Whisperer's `{response_body}` into the *instruction field* of the next agent rather than into a mere data field: `Starter → Whisperer → Parametrizer → <the agent that does the work> → Ender`. Three rules when you draw one. **(1)** Never set `record_seconds` — a spoken instruction has no length anyone can know in advance, and any number cuts the speaker off mid-sentence while still looking like it worked. **(2)** Put a **Forker on `{status}` before the working agent**: `empty` (nobody spoke) and `engine_unavailable` (no recogniser installed) must route to a Notifier/Ender, never onward — feeding a blank or missing transcript into a working agent is how a flow ends up executing something nobody said. **(3)** Keep the working agent a SAFE one unless the operator explicitly accepts otherwise; a canvas flow has no human to confirm an irreversible spoken instruction the way the chat card does.
...
- **Config parameters**:
  ...
  - `record_seconds`: 0 (**THE MODE SWITCH** — 0 = no duration given, so the SOUND GATE listens until the speaker stops; N = record exactly N seconds with the gate off)
  - `silence_timeout_seconds`: 3.5 (seconds of continuous silence that end a gated recording; the clock starts the moment the mic opens)
  - `silence_gate`: "auto" (auto = gate on when record_seconds is 0; on = force the gate even with a duration, which then acts as the ceiling; off = force a fixed recording)
  - `max_record_seconds`: 300 (hard ceiling for a gated recording — a gate with nothing to stop it never stops)
  - `silence_threshold_db`: 0 (0 = measure the room continuously and sit 9 dB above it, never above -50 dBFS; a NEGATIVE value forces a fixed threshold)
...
### 84. Video-Analyzer
- **Purpose**: Analyze recorded videos in three modes: `robotics` (default, conservative motion verdict), `transcription` (speech from audio tracks), or `summary` (visual evidence plus speech and detailed synthesis). Always set `analysis_type` explicitly for content tasks.
- **Robotics**: Camcorder/STM32er physical feedback loops retain the OpenCV motion gate, two independent vision interpreters and merger. `PASS_OK` requires both explicit passes; missing/disagreeing verdicts cannot pass. Branch on `TLM_VERDICT::PASS_OK` or `TLM_VERDICT::FAIL`; handle UNCLEAR/ANALYSIS_ERROR too.
...
- **Starts other agents**: YES, always after structured output, including errors. Large results remain in `report_path`/`analysis_path`; wrapped results flag shortened fields in `promoted_fields_truncated`.
- **Full guide**: `agents/video_analyzer/README.md`.
...
- **Starts other agents**: YES, on success, failure and refusal. Forker can branch on `{status}`, `{page_count}` or `{layout_clean}`. `status: created` proves creation only; check `layout_clean` and `response_body` before delivery. A blank audit field is not a pass, `overlaps: 0` does not exclude other findings, and `repairs` lists attempted layout adjustments rather than certifying the document. PDFer has no `success` contract field.
...
- **Compiler requirement**: PDF compilation needs an installed TeX distribution; MiKTeX is recommended (https://miktex.org/download) for on-demand package installation. TeX Live / MacTeX are also detected; the `acquire` repair rung can attempt installation through an available `tlmgr`. Missing engines produce a structured compilation refusal. `list_styles` works without probing TeX; source authoring also does not require a compiler.
- **Application example**: Starter → upstream agent (produce LaTeX, not Markdown) → Parametrizer (map `{response_body}` into `input_text`) → LaTeXer (`action: compile`, `title: Weekly Report`) → Forker (require `status: compiled` and `success: true`) → downstream delivery → Ender. For complex source, prefer an existing `tex_path` or correctly encoded `input_text_b64`; do not put plain text into a base64 field. A second common shape is Starter → LaTeXer (`action: compile_project`, `project_dir: <a folder of .tex files>`) → Forker (branch on `{success}`) → Ender.
...
- **Compilation result gate**: `compiled` with `success: true` and zero errors is a clean compile; deliver it and stop unless an unmet requirement warrants further work. `compiled_with_errors` and `degraded` both have `success: false` even when `output_path` exists. `degraded` means blocks were removed to rescue a partial PDF; preserve that distinction and route the report for review. Source creation (`created`) and catalog/inspection success do not mean a PDF was compiled. LaTeXer has no normal `layout_clean` field: inspect overflow warnings and any requested visual evidence separately.
- **Repair ladder**: in order, `lint`, `preamble`, `rules`, `log_directed`, `acquire`, `engine_swap`, `model`, `bisect`. Early static fixes precede compilation; subsequent rungs follow failures. Repairs normally use a sibling `.latexer-fixed.tex`, and failing model rewrites are discarded. A model timeout/unreachable endpoint prevents destructive bisect. The compile INI also emits repair/quarantine counts, but the current Parametrizer contract above does not expose `{repairs}` or `{quarantined}` for LaTeXer; branch on `{status}`/`{success}` and read the repair details from `{response_body}`.
- **Config parameters**:
  - `action`: "compile" (compile | compile_project | scaffold_compile | create_file | create_from_template | edit_file | read_file | list_files | validate_tex | structure | clean | validate | install | list_styles)
  ...
  - `input_text_b64`, `content_b64`, `find_text_b64`, `replace_text_b64`: optional UTF-8 base64 counterparts; a supplied, successfully decoded nonempty encoded field takes precedence. Decode failures are logged and retain the literal field. Use them for multiline TeX, tables, TikZ and literal backslashes. Body and metadata are LaTeX, so escape literal special characters; base64 preserves bytes but does not convert prose into TeX.
  ...
  - `style`: "" preserves legacy output. `list_styles` lists 30 identities without requiring TeX: editorial (`scholarly`, `swiss`, `atelier`), playful (`cute_candy`, `baby_blush`, `baby_sky`, `storybook`, `lullaby`), cyberpunk (`cyberpunk`, `neon_noir`, `holographic`, `synthwave`), cosmic (`cosmic_nebula`, `astral_atlas`, `lunar_minimal`, `solar_flare`, `aurora`, `deep_space`), electronics (`circuit_board`, `blueprint`, `quantum_chip`, `oscilloscope`), Tlamatini (`aztec_obsidian`, `quetzal_jade`, `tlamatini_celestial`, `xeno_codex`, `alien_biolume`, `solar_codex`, `obsidian_portal`, `quetzal_supernova`). Styles apply to generated sources: `scaffold_compile`, `create_from_template`, `create_file`, or `compile` with a bare fragment. Omit style for an existing complete .tex/project.
  - `subtitle`: "" / `style_mode`: "screen" (screen | print) / `style_decoration`: "rich" (none | restrained | rich) / `style_cover`: true / `predominant_color`: "" (optional #RRGGBB seed, text contrast corrected afterwards). Metadata and body remain LaTeX. Prefer `content_b64` for complex styled bodies. Styles use portable TikZ artwork, Latin Modern fonts, coordinated body pages and separate Beamer recipes. Beamer scaffolds expect frame-body content, not a complete sequence of frames; use complete source without a style override for explicit frames. `style_cover: false` removes the Beamer illustration while retaining its title frame.
  ...
  - `preflight`: true (fail-safe: REFUSE rather than mis-typeset) / `command_timeout`: 600 (per external tool invocation, not the whole multi-pass/repair run) / `max_log_chars`: 20000
...
### 89. PPTXer
- **Purpose**: AUTHOR a PowerPoint presentation. PPTXer is the DECK COMPOSER — the third member of the document family (PDFer COMPOSES a PDF, LaTeXer TYPESETS one, PPTXer BUILDS the slides).
- **Used for**: Turning text a previous agent produced — a Summarizer digest, a File-Interpreter reading, a Crawler result, Tlamatini's own answer — into ONE finished `.pptx` carrying embedded images (including images fetched from the INTERNET), video, audio, native editable diagrams, real tables and generated artwork. It needs NO installation: python-pptx + Pillow already ship with Tlamatini.
- **Aimed at**: The LAST hop of a presentation flow, and built above all for MARKETING and GAMING work — launch decks, tournament recaps, trailer beat-sheets, pitch decks, campaign reports, status reviews. Choose PPTXer when the deliverable is something a person will PRESENT; choose PDFer when it is something they will READ; choose LaTeXer when it needs real mathematics. NEVER hand-roll a `.pptx` through Executer/Pythonxer.
- **Application example**: Starter → Crawler (gather the launch material) → Parametrizer (map `{response_body}` into PPTXer's `input_text`) → PPTXer (`nuance: esports_tournament`, `predominant_color: "#12E2A3"`) → Forker (check status, audit evidence and required media) → downstream delivery → Ender. A second common shape is Starter → Shoter → Parametrizer (map `{output_path}` into PPTXer's `images`) → PPTXer → Ender.
...
- **Result gate and rendering**: `success: true` also accompanies `created_with_findings`, so it is insufficient for delivery. Check `layout_clean`, `response_body` confidence, skipped/error measurements, and `slides_rendered` against `slide_count` when claiming full rendering. Header `ground_truth` identifies the renderer, not a complete clean audit. Missing audit results do not pass. `render: auto` tries PowerPoint, LibreOffice and then an approximate Pillow preview; native apps are optional for creating a deck. `render: none` skips slide images, while `layout_audit: true` still checks geometry. Required omitted media must be reported. `outline`/`info`/`fonts` return `listed`, `validate` returns `validated`, and none creates a new deck; read the validate report for unavailable optional backends. Existing-deck `render`/`audit` return `ok` or `findings`.
- **THE LOOK IS COMPUTED FROM THE CONTENT**: PPTXer classifies the deck before building it, so in most flows you set NOTHING and still get a treatment that suits the subject — a tournament deck comes out loud and neon, an investor pitch confident and clean, a safety briefing plain with no ornament at all. Two overrides matter when you are designing a flow deliberately:
  - `nuance` — force an original content treatment (`product_launch`, `esports_tournament`) or an explicit visual style (`blueprint`, `botanical`, `swiss_editorial`). Empty retains automatic classification. There is no separate `style` parameter or style-catalog action; use the linked style guide.
...
- **Config parameters**:
  ...
  - `accent_color` / `text_color` / `background_color` / `heading_color` / `link_color` / `table_header_color` / `rule_color`: "" (per-role overrides; each wins over the nuance and over `predominant_color` before final contrast correction) / `background_mode`: "auto" (auto | dark | light)
  ...
  - `slide_numbers`: true / `footer_note`: "" / `notes`: "" (deck-level speaker notes)
  - `ollama_design`: false / `ollama_outline`: false (both OFF by default — the deterministic path is already complete, validated and audited, so the model is an upgrade rather than a dependency, and a deck that renders differently every time is one nobody trusts) / `ollama_url` / `ollama_model` / `ollama_token` / `ollama_timeout`: 180
  - `output_dir`: "" (empty = Documents/TlamatiniPPTX) / `filename`: "" (empty = a timestamped name) / `overwrite`: false
