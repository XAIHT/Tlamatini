...
20. **The wrapped spec's `purpose` / `example_request` IS the contract the model obeys — changing an agent's behaviour without changing that text ships an invisible feature.** A wrapped chat-agent's defaults reach the LLM only through the prose in `chat_agent_registry.py`. When Whisperer's microphone capture changed from a fixed `record_seconds: 30` to a silence-gated recording (2026-09-11), the description still said *"records record_seconds (default 30)"* and the worked example still opened with *"Transcribe 5 seconds…"* — so Tlamatini would have kept volunteering a duration out of habit, the gate would never have fired, and the change would have been **invisible while looking like it worked**. Rewrite the spec text in the SAME pass, state plainly what the model must NOT pass, and pin the sentence with a test. The same rule applies to a new sentinel value: if `0` now means "auto", say so where the model reads it, not only in `config.yaml`.
21. **A result field that can differ from its input must be computed from the artefact.** Whisperer's `duration_seconds` echoed the *requested* length; once the length became condition-driven, that field was simply untrue. Derive such fields from the thing produced (the captured buffer, the file on disk) and add a `stop_reason`-style field so a downstream Forker can branch on WHY it ended. New `INI_SECTION` fields are **APPENDED, never renamed**, and `agent_contracts._PARAMETRIZER_OUTPUT_FIELDS` is updated in the same commit (`views.PARAMETRIZER_SOURCE_OUTPUT_FIELDS` is DERIVED from it — never hand-edit that one). Membership in `parametrizer.py::SECTION_AGENT_TYPES` is a one-time registration: the generic KV-header parser picks up appended fields with no further change.
