...
│   │   │
│   │   ├── agents/                 # Workflow agent templates (59 types)
...
│   │   │   ├── whatstlamatini/ # Long-running WhatsApp Cloud-API bridge for the full Tlamatini chat (Multi-Turn + Exec Report)
...
| **whatstlamatini** | Long-running active agent that exposes the full Tlamatini chat (same Multi-Turn + Exec Report behavior as `agent_page.html`) over **WhatsApp**, via Meta's WhatsApp Cloud API. Mirror of `teletlamatini` — same persistent Tlamatini WebSocket bridge (one HTTP login at startup, reused for every WhatsApp message), same per-chat password gate, same optional Ollama-backed completeness classifier, same Multi-Turn + Exec Report payload, same `target_agents` cycle. Replaces Telegram-specific bits with: a stdlib HTTP webhook listener for inbound messages (the user must expose the URL publicly via ngrok / cloudflared / domain / port-forward and register it in Meta App → Webhooks) and outbound HTTP POSTs to the Graph API. WhatsApp text messages are not editable, so the answer is delivered as a separate "✅ Result:" reply rather than an in-place edit. | `whatsapp.phone_number_id`, `whatsapp.access_token`, `whatsapp.verify_token`: Meta WhatsApp Cloud API credentials (from WhatsApp Manager → API Setup)<br>`whatsapp.webhook_host`, `whatsapp.webhook_port`, `whatsapp.webhook_path`: where the inbound webhook listener binds (must be exposed publicly)<br>`password`: Password the WhatsApp user must supply on first contact (empty disables the gate)<br>`tlamatini.base_url`: Tlamatini server endpoint (`ws_url` auto-derived)<br>`tlamatini.username`, `tlamatini.password`: Tlamatini Django credentials<br>`tlamatini.multi_turn_enabled`, `tlamatini.exec_report_enabled`: Always-on toggles for the proxied chat<br>`tlamatini.response_idle_timeout`, `tlamatini.total_timeout`: WebSocket read deadlines<br>`completeness_check.enabled`, `completeness_check.host`, `completeness_check.model`, `completeness_check.instruction`: Optional Ollama-backed clarification gate (off by default)<br>`source_agents`: Informational only<br>`target_agents`: Downstream agents launched after every completed user request cycle |
...
Each spec carries a **transport profile** that controls how `runtime.send_turn` drives the child process:

- **`oneshot-prompt`** (claude / cursor / codex / gemini / qwen) — re-spawn the CLI fresh on each turn with the prompt as a CLI argument behind `prompt_arg_flag` (`-p` for claude / cursor / gemini / qwen) or `prompt_subcommand_args` (`["exec"]` for codex), close stdin, and capture stdout to EOF via `proc.communicate(timeout=180)`. **This is the only transport that reliably captures TUI agents' answers on Windows** — interactive TUIs detect a piped stdout and refuse to flush, so a long-lived stdin-fed child captures only the outbound prompt. The CLI's own non-interactive mode (`claude -p "<task>"`, `gemini -p "<task>"`, `codex exec "<task>"`, ...) is what each vendor ships for unattended use; ACPX leans on it directly. **No inter-turn child state**: each `acp_send` is a fresh process, so conversation continuity must be carried in the next prompt.
- **`json-acp`** (tlamatini self-host) — child speaks the strict ACP envelope on stdout; drain waits for `done:true`.
- **`tui-repl`** (kiro / kimi / iflow / kilocode / opencode / pi / droid / copilot) — long-lived interactive REPL whose one-shot flag we don't yet know. May emit zero stdout per turn when fed JSON on a piped stdin (drain uses the transport-aware idle rule with tight TUI defaults). `acp_spawn` returns the `session_id` immediately (sub-second) without draining; harvest content via the next `acp_send` / `acp_send_and_wait` / `acp_transcript`. Override any of these to `oneshot-prompt` per-agent in `config.json` once the CLI's non-interactive flag is confirmed.

| `agent_id` | Default command | Transport | Prompt argv form | Default budgets (timeout/idle/grace) | Description |
|------------|-----------------|-----------|------------------|--------------------------------------|-------------|
| `claude` | `claude` | `oneshot-prompt` | `claude -p "<task>"` | 180 s / 10 s / 2 s | Anthropic Claude Code CLI |
| `codex` | `codex` | `oneshot-prompt` | `codex exec "<task>"` | 180 s / 10 s / 2 s | OpenAI Codex CLI |
| `cursor` | `cursor-agent` | `oneshot-prompt` | `cursor-agent -p "<task>"` | 180 s / 10 s / 2 s | Cursor agent CLI |
| `gemini` | `gemini` | `oneshot-prompt` | `gemini -p "<task>"` | 180 s / 10 s / 2 s | Google Gemini CLI |
| `qwen` | `qwen-code` | `oneshot-prompt` | `qwen-code -p "<task>"` | 180 s / 10 s / 2 s | Alibaba Qwen Code CLI |
| `tlamatini` | `<sys.executable> -m agent.acpx.self_acp_server` | `json-acp` | (stdin envelope) | 45 s / 6 s / 12 s | Tlamatini-as-ACP-server (self-host, reserved slot) |
| `copilot` | `copilot` | `tui-repl` | (stdin) | 8 s / 2 s / 3 s | GitHub Copilot CLI |
| `pi` | `pi` | `tui-repl` | (stdin) | 8 s / 2 s / 3 s | Pi assistant CLI |
| `droid` | `droid` | `tui-repl` | (stdin) | 8 s / 2 s / 3 s | Factory Droid CLI |
| `iflow` | `iflow` | `tui-repl` | (stdin) | 8 s / 2 s / 3 s | iFlow CLI |
| `kilocode` | `kilocode` | `tui-repl` | (stdin) | 8 s / 2 s / 3 s | Kilocode CLI |
| `kimi` | `kimi` | `tui-repl` | (stdin) | 8 s / 2 s / 3 s | Kimi CLI |
| `kiro` | `kiro` | `tui-repl` | (stdin) | 8 s / 2 s / 3 s | Kiro CLI |
| `opencode` | `opencode` | `tui-repl` | (stdin) | 8 s / 2 s / 3 s | OpenCode CLI |

You can override any default and add new entries through `acpx.agents` in `config.json` (see [ACPX Settings](#acpx-settings-external-coding-agent-runtime--skills) for the full schema). Custom user-defined `agent_id`s default to `tui-repl` with the fast-path defaults — set `transport: oneshot-prompt` and `prompt_arg_flag: "-p"` (or whatever flag your CLI uses) to opt them into the capture path.
...
| 21 | `setup-new-acpx-key` | in-process | End-to-end credential setup for a registered ACPX `agent_id`: writes `data.keys`, patches both `config.json` layers (top-level + `acpx.agents.<id>.env`), optionally extends `regen_secrets.py`, then verifies via `acp_doctor`. Canonical env-var map: claude → `ANTHROPIC_API_KEY`, gemini → `GEMINI_API_KEY` (+ `GOOGLE_API_KEY` alias), codex → `OPENAI_API_KEY`, qwen → `DASHSCOPE_API_KEY` |

Adding skill #22 means dropping a new directory under `agent/skills_pkg/` — no Python edits required. The registry has a 30-second staleness cap and reloads on demand.
...
| `/update_whatstlamatini_connection/<agent_name>/` | POST | Update whatstlamatini connections |
...
| **WhatsTlamatini** | WhatsApp counterpart of TeleTlamatini. Long-running active agent that exposes the full Tlamatini chat (`agent_page.html` Multi-Turn behavior, including the per-agent Exec Report tables) over WhatsApp via Meta's WhatsApp Cloud API. Runs a stdlib HTTP webhook listener on a configurable host:port path (the user exposes that URL publicly via ngrok / cloudflared / domain / port-forward and registers it in Meta App → Webhooks); inbound messages are dispatched per-sender through the same persistent Tlamatini WebSocket bridge, password-gated on first contact, optionally run through an Ollama-backed completeness classifier, then proxied into Tlamatini with `multi_turn_enabled=true` and `exec_report_enabled=true`. Outbound replies are HTTP-POSTed to `https://graph.facebook.com/v20.0/<phone_number_id>/messages`. WhatsApp text messages are not editable, so the answer is delivered as a separate "✅ Result:" reply. After every successfully completed user request cycle, starts the configured `target_agents` |
...
- **ACPX Toolbar Toggle (Per-Request Enable/Disable) — May 2026** — The chat toolbar now exposes three checkboxes side-by-side: **Multi-Turn**, **Exec Report**, and **ACPX**. The new ACPX checkbox (`#acpx-enabled` in `agent/templates/agent/agent_page.html`) sends an `acpx_enabled` boolean on every WebSocket request. `agent/rag/interface.py::ask_rag` reads it (defaulting to `True` for object payloads), pipes it into the chain payload via the `UnifiedAgentChain.invoke()` whitelist, and the planner / executor call `agent.acpx.filter_acpx_tools(tools, acpx_enabled)` to strip the entire 12-tool ACPX/Skill surface (`acp_spawn`/`acp_send`/`acp_send_and_wait`/`acp_kill`/`acp_doctor`/`acp_transcript`/`acp_session_status`/`acp_list_sessions`/`acp_relay`/`list_acp_agents`/`invoke_skill`/`list_skills`) from the bound tool list when the flag is `False`. Net result: a Multi-Turn request with ACPX unticked behaves exactly like the legacy pre-ACPX Multi-Turn flow — the planner never even sees the ACPX tools, so the user can fall back to native chat-agent flows on demand without a server restart. `bypass_prompt_validation` is computed as `multi_turn_enabled OR acpx_enabled`, so the system still skips prompt-shape validation when only ACPX is checked (correct, because ACPX flows are LLM-operator flows, not Q&A). Files touched: `agent/acpx/__init__.py` (new `ACPX_TOOL_NAMES` frozenset + `filter_acpx_tools`), `agent/rag/interface.py`, `agent/rag/chains/unified.py`, `agent/rag/factory.py`, `agent/consumers.py`, `agent/mcp_agent.py`, `agent/templates/agent/agent_page.html`, `agent/static/agent/js/agent_page_init.js`, `agent/static/agent/js/agent_page_state.js`.

- **Summarizer One-Shot Mode + `chat_agent_summarize_text` Tool — May 2026** — `agent/agents/summarizer/summarizer.py` now accepts a one-shot path: when `input_text` is non-empty AND `source_agents` is empty, the agent skips the polling loop entirely, sends `input_text` directly to the LLM with the resolved prompt (built from `target_words` if no `system_prompt` is provided), emits exactly one `INI_SECTION_SUMMARIZER<<<` block (so Parametrizer / Exec Report consume the result identically to a polling-mode summary), and triggers `target_agents` whenever the summary is non-empty. The wrapped chat tool `chat_agent_summarize_text` (registered in `chat_agent_registry.py`, `template_dir="summarizer"`) is the canonical caller — its `example_request` is `Summarize with input_text='<full text>' and target_words=40`. Pre-existing canvas behavior (polling `source_agents` for `[EVENT_TRIGGERED]`) is unchanged when `input_text` is left at its default empty string. The agent's own log shows `One-shot input_text length: <N> chars; target_words=<M>` so a quick grep of `summarizer_<n>.log` confirms which path fired.

- **`setup-new-acpx-key` Skill — May 2026** — A new in-process skill (`agent/skills_pkg/setup_new_acpx_key/SKILL.md`, registered as skill #21) that walks the LLM through credential injection for any `agent_id` from the ACPX registry end-to-end: writes `data.keys`, patches both layers of `config.json` (top-level for callers like `image_interpreter.py` / `opus_client.py`; `acpx.agents.<id>.env` for the spawned child), optionally extends `regen_secrets.py` when introducing a brand-new key, then verifies via `acp_doctor`. Documents the canonical env-var map (claude → `ANTHROPIC_API_KEY`, gemini → `GEMINI_API_KEY` + `GOOGLE_API_KEY` alias, codex → `OPENAI_API_KEY`, qwen → `DASHSCOPE_API_KEY`) and the `{**os.environ, **spec.env}` merge order so explicit `acpx.agents.<id>.env` always wins over an exported shell variable. Frontmatter declares `requires_tools: [chat_agent_file_creator, chat_agent_executer, acp_doctor]` and `requires_mcps: [Files-Search]`, so the harness pre-validates the necessary surfaces before invocation.

- **`regen_secrets.py` Two-Mode Scrubber/Restorer — May 2026** — A new repo-root script (`regen_secrets.py`) that toggles `Tlamatini/agent/config.json` between two states: `--mode push-able` rewrites real secrets (top-level `ANTHROPIC_API_KEY` / `GEMINI_API_KEY` / `OLLAMA_TOKEN` and the `acpx.agents.<id>.env` blocks) into `<KEY goes here>` placeholders so the file is safe to commit; `--mode keyed` restores values from `data.keys` (gitignored, `KEY=VALUE` format) so the local working tree stays usable. Same script is the splat target after editing `data.keys`. The `setup-new-acpx-key` skill patches this script when a brand-new key is introduced (adds a `set_top` / `set_acpx_env` rule and updates the docstring). `data.keys` is gitignored at line 265 of `.gitignore` — never commit it.

- **Console Window + App-Icon Polish, Restrictive-Policy-Friendly Shortcut — May 2026** — `Tlamatini/manage.py` now sets the console window title to `Tlamatini` and binds the `Tlamatini.ico` (under `agent/static/agent/img/`) so the running console looks like a real Windows app. The same icon is wired into `build.py` (PyInstaller exe icon), `templates/agent/{login,welcome,agent_page,agentic_control_panel}.html` (favicon for every web page), and `register_flw.ps1` (file-association icon for `.flw`). `CreateShortcut.ps1` was rewritten over four iterations to work on machines with restrictive Group Policies — when `-NoCreatePublic` is enforced (no Public Desktop / All-Users Start Menu writes), the script now falls back to the user-scoped `$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Tlamatini.lnk` and `[Environment]::GetFolderPath('Desktop')\Tlamatini.lnk` paths automatically, so the installer no longer fails on locked-down corporate boxes. Same script also gained an `-Optimized` flag that skips redundant icon-cache busting steps when the user already has the shortcut.

- **ACPX `oneshot-prompt` Transport — Capturing Claude / Gemini / Cursor / Qwen / Codex Responses on Windows — May 2026** — The previous transport-aware drain landed sub-second spawns and a fast idle rule, but on Windows the actual answer text from TUI agents (Gemini CLI, Cursor, Qwen, Claude Code) was **never captured**. Their JS-based TUIs detect a piped stdout and refuse to flush — so the transcript only carried the outbound prompt and the LLM had to apologize with "the transcript only shows the outbound prompts, not the inbound responses". Fix:

    1. **New `transport="oneshot-prompt"` profile.** `AcpAgentSpec` gains two fields: `prompt_arg_flag: Optional[str]` (e.g. `"-p"`) and `prompt_subcommand_args: List[str]` (e.g. `["exec"]` for codex). Each turn now re-spawns the CLI fresh with the prompt as a CLI argument: `claude -p "<task>"`, `gemini -p "<task>"`, `cursor-agent -p "<task>"`, `qwen-code -p "<task>"`, `codex exec "<task>"`. stdin is closed immediately, and `proc.communicate(timeout=180)` captures stdout and stderr to EOF — the same non-interactive mode each CLI ships for unattended use.

    2. **Five agents migrated.** `claude` and `codex` moved from `json-acp` → `oneshot-prompt`. `cursor`, `gemini`, `qwen` moved from `tui-repl` → `oneshot-prompt`. The remaining 8 `tui-repl` agents (`kiro`, `kimi`, `iflow`, `kilocode`, `opencode`, `pi`, `droid`, `copilot`) keep their fast-path defaults; users can override per-agent in `config.json.acpx.agents.<id>` once their non-interactive flag is confirmed. `tlamatini` self-host stays on `json-acp` (it speaks the protocol natively).

    3. **Generous defaults for LLM answer time.** `oneshot-prompt` agents get **180 s timeout / 10 s idle / 2 s grace** (vs the previous 45 s/8 s for the other transports) because LLM responses can take >2 minutes for a long-form answer. The hard timeout is the only meaningful budget — `idle` and `grace` are effectively unused for this transport since the runtime drains until child exit.

    4. **`AcpSession._oneshot_send_turn` mirrors the existing event contract.** Yields one `assistant_message` event with `role:"assistant"`, `text:<stdout>`, `exit_code`, `elapsed_seconds`, then optionally one `log` event with `channel:"stderr"` if stderr was non-empty, then a synthetic `done` (`child_exited` or `timeout`). `extract_last_assistant_text` picks up the assistant_message verbatim, so `acp_relay`, `acp_transcript`, and the visual ACPXer's INI_SECTION_ACPXER body all see the captured answer with no further wiring.

    5. **No inter-turn child state.** Each `acp_send` is a brand-new CLI invocation, so conversation continuity inside the child is gone — caller must include prior context in the next prompt if needed. This matches how each CLI's `-p` mode works in the user's own shell, and is the price for actually capturing output. Live multi-turn conversations should use `acp_send_and_wait` with a single appended history.

    6. **ACPXer canvas counterpart updated in lockstep.** `agent/agents/acpxer/acpxer.py` mirrors the new transport in its inline registry (the visual workflow agent runs as a subprocess and cannot import `agent.acpx`, by design — see the existing `agent.*`-from-pool gotcha). New `run_oneshot_prompt()` helper does the fresh-process-per-turn capture; `extract_last_assistant_text` now prefers envelope-marked `assistant_message` events and skips `channel:"stderr"` logs so the harvested `response_body` is the LLM answer alone, not assistant-text + stderr noise.

    7. **Coverage.** `agent/acpx/tests.py::OneshotPromptCaptureTests` (4 tests) exercise the new path against a `python -c` stand-in: stdout captured as `assistant_message`, transcript carries both `out` and `in` lines, `extract_last_assistant_text` returns the answer alone (skipping stderr logs), and a missing CLI yields a clean `command_not_found` synthetic event. `AgentRegistryTransportProfileTests` rewritten: new `test_oneshot_prompt_agents_have_capture_path` pins `transport="oneshot-prompt"` + correct `prompt_arg_flag` / `prompt_subcommand_args` for all five CLIs; `test_legacy_tui_agents_keep_fast_path_defaults` and `test_json_acp_self_host_keeps_blocking_drain` cover the other two transports. **65 ACPX tests green, 9 ExecReport tests green, 0 regressions, ruff clean.** Files touched: `agent/acpx/agent_registry.py`, `agent/acpx/runtime.py`, `agent/acpx/tests.py`, `agent/agents/acpxer/acpxer.py`, `agent/prompt.pmt`, `agent/agents/flowcreator/agentic_skill.md`, `docs/claude/acpx.md`, `docs/claude/gotchas.md`, `README.md`.
...
- **Added WhatsTlamatini Agent** - WhatsApp counterpart of TeleTlamatini. Long-running active agent that exposes the full Tlamatini chat (`agent_page.html` Multi-Turn behavior, including the per-agent Exec Report tables) over **WhatsApp**, via Meta's WhatsApp Cloud API (`https://graph.facebook.com/v20.0/<phone_number_id>/messages`). Reuses TeleTlamatini's persistent Tlamatini WebSocket bridge verbatim (one HTTP login at startup, reused for every inbound WhatsApp message), per-chat phase state machine (AWAIT_PASSWORD / READY / PROCESSING / AWAIT_INFO), reanim-aware state, and optional Ollama-backed completeness classifier. Inbound messages arrive via a stdlib `http.server` ThreadingHTTPServer running in a daemon thread on a configurable host:port path — the user exposes that URL publicly (ngrok / cloudflared / domain / port-forward) and registers it as the app's webhook in Meta App → Webhooks (subscribed to the `messages` field). Outbound replies are HTTP-POSTed to the Graph API. WhatsApp text messages are not editable, so the answer is delivered as a separate "✅ Result:" reply rather than an in-place edit (only protocol-level UX divergence vs. TeleTlamatini). Unique 4-color gradient: Verdant Mirage — deep-emerald → vivid-emerald → tropical-cyan → goldenrod, distinct from `.whatsapper-agent` (plain DarkGreen), `.teletlamatini-agent` (Telegram blues), and `.acpxer-agent` (cosmic-navy/violet/magenta/cyan). Migration `0077_add_whatstlamatini.py` seeds the `Agent` row.
