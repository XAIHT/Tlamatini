...
The PDFer upgrade landed after the `v1.51.0` tag and is carried by `v1.51.1` and later. Its **nuance / typography / layout overhaul** adds ten flat sibling modules that read the content first and pick one of twenty design treatments, derive a full 38-role palette from a single colour in OKLab, register the host's TrueType families, solve table column widths from real font metrics so cells cannot overlap, draw their own on-palette artwork, and then re-open the finished PDF to measure it. The same dossier window added 15 small PDFer ornament PNGs and 18 text files; those new text assets total **10,512 physical lines / 7,053 effective lines** before this documentation refresh.
...
**🛠️ Code & projects**
- **PDFer** — the **document composer**: turn Tlamatini's own answer, some Markdown/HTML, plain text, a folder of images, or several existing PDFs into ONE styled PDF — with a cover page, real tables, page numbers and an optional table of contents. It is the WRITE side of the document family (File-Extractor / File-Interpreter *read* documents; PDFer *authors* them). **Needs no installation** — every engine it uses already ships inside Tlamatini. Modes: `auto` (it sniffs the content for you) / `markdown` / `html` / `text` / `images` (one-per-page, fit, or grid) / `mixed` (prose + embedded figures) / `merge` / `info` / `validate`. **It reads what you wrote and decides how the document should look, before it renders anything.** A piece about science or technology comes out on a near-black page with white type, cyan-to-violet gradients and a generated star field; a document with an abstract and citations comes out pure white, black and justified, set in Palatino the way a journal typesets an article; a contract comes out plain, in Times, with no decoration whatsoever. Twenty treatments in all, chosen from the vocabulary, the structure and the tone of your text — and you can override it with **`nuance`** (`science`, `paper`, `legal`, `receta` … any alias, English or Spanish) or hand it a single **`predominant_color`** and watch the *whole* palette — headings, rules, table headers, callout bars, the cover gradient, the footer — be derived from that one colour. **Its tables cannot overlap.** Every column width is solved from the real font metrics before a letter is placed, so a long Windows path or a URL wraps inside its own cell instead of printing across the next one; and once the file is written PDFer re-opens it and measures the actual letters on the page for overlapping text, ink running off the sheet, blank pages and real contrast, so it can tell you the layout was *verified* rather than assumed. **It draws its own illustrations** — gradient washes, star fields, circuit traces, waves, soft organic shapes — in your document's own colours, with no image files and no downloads, and only where the content makes that safe: a contract, a dosage chart and a financial statement get none. Optionally let an Ollama model tidy the text into clean Markdown first, or art-direct the colours for you — both off by default, and neither can ever lose your document. PDFs land in **Documents/TlamatiniPDF** with a collision-proof name, and a fail-safe preflight refuses rather than write an empty or wrong file.
- **PPTXer** — the **deck builder**: turn Tlamatini's own answer, some Markdown, or a plain outline into ONE finished PowerPoint presentation — a cover, section breaks, bullet slides, big stat tiles, real tables, pull-quotes, diagrams and a closing slide. It is the third member of the document family (PDFer *composes* a PDF, LaTeXer *typesets* one, PPTXer *builds the slides*), and like them it **needs no installation** — everything it uses already ships inside Tlamatini. It is built above all for **marketing and gaming**: a tournament recap comes out loud and neon set in Nasalization, a cyberpunk brief comes out in Terminator with circuitry traced behind it, an investor pitch comes out confident and clean, and a safety briefing comes out plain with no decoration at all — twenty-four treatments in all, chosen from your own words, and overridable with **`nuance`** or a single **`predominant_color`** that the whole palette is then derived from. **It can pull pictures off the internet**: give it image URLs (or just write `![](https://…)` in your Markdown) and it fetches them, checks what they really are by their bytes rather than trusting what the server claims, and embeds them in the file — and it will embed video and audio too. **And it looks at the slides it just made.** PowerPoint files are easy to write and easy to get wrong: the library that writes them does no layout at all, so “it saved without an error” tells you nothing about whether the text fits. So PPTXer measures every word in the real typeface before it places anything, then opens the finished deck in **your installed PowerPoint**, exports every slide as a picture, and measures those pixels — for text that overlaps, text that has spilled outside its box, anything hanging off the edge of the slide, and whether the words are actually readable against whatever ended up behind them. If it finds something, it says so plainly instead of calling the deck finished. Decks land in **Documents/TlamatiniPPTX** with a collision-proof name, and a fail-safe preflight refuses rather than write an empty or wrong file.
