...
2. **Single source of truth**: a **git tag** of the form `v1.50.0`.
...
4. **Three injection points**, all computed automatically at build time:
   ...
   - The release folder name (`dist/Tlamatini_Release_v1.50.0/`)
...
build.py / build_installer.py / build_uninstaller.py
   ...
   ├─►  dist/Tlamatini_Release_v1.50.0/      (release folder name)
   └─►  Runtime surfaces:
            ├── About dialog:   "Tlamatini v1.50.0"
            ├── Console banner: "--- [VERSION] Tlamatini 1.50.0"
...
|---|---|---|---|
| 1 | **`--version X.Y.Z` CLI flag** | `python build.py --version 1.50.0` | `1.50.0` |
| 2 | **`$env:TLAMATINI_VERSION`** | `$env:TLAMATINI_VERSION = "1.50.0"; python build.py` | `1.50.0` |
| 3 | **`git describe --tags --abbrev=0 --match 'v[0-9]*'`** | `git tag -a v1.50.0 -m "..."; python build.py` | always the bare base tag → `1.50.0` (distance / dirty state never appear in the version string) |
...
The commands below show the current `v1.50.0` release as a concrete example. That tag already exists: **do not rerun these commands, move it, or recreate it**. Substitute the next deliberate version when preparing a future release.

```powershell
git tag -a v1.50.0 -m "Release 1.50.0: <one-line summary>"
```
...
All three scripts pick up `v1.50.0` automatically (precedence #3 — Git finds the newest reachable annotated tag). This remains true when `HEAD` is a later untagged commit, because Tlamatini deliberately emits the bare base tag without a distance or dirty suffix.

You'll see this in each script's output:
```
Tlamatini version : 1.50.0
...
The final artefact is **`dist/Tlamatini_Release_v1.50.0/`** — zip and distribute.
...
- **About dialog**: `Tlamatini v1.50.0`
- **Right-click `Tlamatini.exe` → Properties → Details**: ProductVersion = `1.50.0`
- **Console banner on startup**: `--- [VERSION] Tlamatini 1.50.0`
- **`curl http://localhost:8000/agent/version/`** (after login or with anonymous access since it's open):
  ```json
  {"version":"1.50.0","build":"1.50.0","commit":"abc1234","date":"2026-08-23T12:00:00Z","source":"generated"}
  ```
...
TAG IT                git tag -a v1.50.0 -m "Release 1.50.0"
                      git push origin v1.50.0
...
WHERE IT LANDS        dist/Tlamatini_Release_v1.50.0/
                      About dialog : "Tlamatini v1.50.0"
                      Exe → Properties → Details : ProductVersion = 1.50.0
                      curl /agent/version/ : {"version":"1.50.0", …}
                      Console banner : --- [VERSION] Tlamatini 1.50.0
