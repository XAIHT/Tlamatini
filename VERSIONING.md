...
2. **Single source of truth**: a **git tag** of the form `v1.49.1`.
...
4. **Three injection points**, all computed automatically at build time:
   ...
   - The release folder name (`dist/Tlamatini_Release_v1.49.1/`)
...
build.py / build_installer.py / build_uninstaller.py
   ...
   ├─►  dist/Tlamatini_Release_v1.49.1/      (release folder name)
   └─►  Runtime surfaces:
            ├── About dialog:   "Tlamatini v1.49.1"
            ├── Console banner: "--- [VERSION] Tlamatini 1.49.1"
...
|---|---|---|---|
| 1 | **`--version X.Y.Z` CLI flag** | `python build.py --version 1.49.1` | `1.49.1` |
| 2 | **`$env:TLAMATINI_VERSION`** | `$env:TLAMATINI_VERSION = "1.49.1"; python build.py` | `1.49.1` |
| 3 | **`git describe --tags --abbrev=0 --match 'v[0-9]*'`** | `git tag -a v1.49.1 -m "..."; python build.py` | always the bare base tag → `1.49.1` (distance / dirty state never appear in the version string) |
...
The commands below show the current `v1.49.1` release as a concrete example. That tag already exists: **do not rerun these commands, move it, or recreate it**. Substitute the next deliberate version when preparing a future release.

```powershell
git tag -a v1.49.1 -m "Release 1.49.1: <one-line summary>"
```
...
All three scripts pick up `v1.49.1` automatically (precedence #3 — Git finds the newest reachable annotated tag). This remains true when `HEAD` is a later untagged commit, because Tlamatini deliberately emits the bare base tag without a distance or dirty suffix.

You'll see this in each script's output:
```
Tlamatini version : 1.49.1
...
The final artefact is **`dist/Tlamatini_Release_v1.49.1/`** — zip and distribute.
...
- **About dialog**: `Tlamatini v1.49.1`
- **Right-click `Tlamatini.exe` → Properties → Details**: ProductVersion = `1.49.1`
- **Console banner on startup**: `--- [VERSION] Tlamatini 1.49.1`
- **`curl http://localhost:8000/agent/version/`** (after login or with anonymous access since it's open):
  ```json
  {"version":"1.49.1","build":"1.49.1","commit":"abc1234","date":"2026-08-23T12:00:00Z","source":"generated"}
  ```
...
TAG IT                git tag -a v1.49.1 -m "Release 1.49.1"
                      git push origin v1.49.1
...
WHERE IT LANDS        dist/Tlamatini_Release_v1.49.1/
                      About dialog : "Tlamatini v1.49.1"
                      Exe → Properties → Details : ProductVersion = 1.49.1
                      curl /agent/version/ : {"version":"1.49.1", …}
                      Console banner : --- [VERSION] Tlamatini 1.49.1
