...
2. **Single source of truth**: a **git tag** of the form `v1.3.0`.
...
build.py / build_installer.py / build_uninstaller.py
   ...
   ├─►  dist/Tlamatini_Release_v1.3.0/       (release folder name)
   └─►  Runtime surfaces:
...
All three scripts will pick up `v1.3.0` automatically (precedence #3 — git describe finds an exact tag at HEAD).

You'll see this in each script's output:
```
Tlamatini version : 1.3.0
...
The final artefact is **`dist/Tlamatini_Release_v1.3.0/`** — zip and distribute.
...
- **About dialog**: `Tlamatini v1.3.0`
- **Right-click `Tlamatini.exe` → Properties → Details**: ProductVersion = `1.3.0`
- **Console banner on startup**: `--- [VERSION] Tlamatini 1.3.0`
- **`curl http://localhost:8000/agent/version/`** (after login or with anonymous access since it's open):
  ```json
  {"version":"1.3.0","build":"1.3.0","commit":"abc1234","date":"2026-05-15T18:42:11Z","source":"generated"}
  ```
...
git tag -a v1.3.0 -m "Release 1.3.0" # re-create at correct commit
git push origin v1.3.0
```

A safer flow: **never delete tags**. If you tagged `v1.3.0` at the wrong commit, bump to `v1.3.1` at the right commit and move on. SemVer was designed assuming you'd do exactly this.
...
TAG IT                git tag -a v1.3.0 -m "Release 1.3.0"
                      git push origin v1.3.0
...
WHERE IT LANDS        dist/Tlamatini_Release_v1.3.0/
                      About dialog : "Tlamatini v1.3.0"
                      Exe → Properties → Details : ProductVersion = 1.3.0
...
UNDO A TAG            git tag -d v1.3.0
                      git push origin :refs/tags/v1.3.0
                      (… but prefer: just bump to v1.3.1 instead)
```
