...
341b. **A NEW SECTION COSTS TWO PROMPTS (2026-09-12, VOICE COMMANDS).** Adding a category means (a) a `('<key>', '<Label>')` entry in `views.PROMPT_CATEGORY_ORDER` at the position you actually want (`voice_commands` is at **index 0**, ahead of Getting Started, by Angela's instruction) AND (b) a **Step-by-Step opener at `sort_rank` 10**, added to `expected_first` in `test_prompt_catalog_contiguous.py` in the same pass. Two tests enforce it: `test_known_section_openers` (a live section absent from the map) and `test_every_section_opens_with_a_genuine_step_by_step_wizard` (the opener's CONTENT must name the Step-by-Step checkbox and promise to WAIT). Nothing else changes — the catalog modal is data-driven, so no JS/CSS/template edit and no `STATIC_VERSION` bump. Frozen historical migrations that mirror the order (`0179`'s `_CATEGORY_ORDER`) stay untouched.
341c. **THE MODE BADGES ARE DERIVED FROM YOUR TEXT — you cannot set them.** `tools_dialog.js::classifyPromptModes` scrubs every `do not use …` clause FIRST, then scans: an `acp_*` / `invoke_skill` / `list_skills` token ⇒ **ACPX** (implies Multi-turn); `chat_agent_*` or "multi-turn" ⇒ **Multi-turn**; the **hyphenated** `step-by-step` token (near tick/check/enable, or followed by wizard/checkbox/mode/…) ⇒ **Step-by-Step**; every non-one-shot card also gets **Exec-report**. So: to make a card ACPX you must NAME an ACPX tool in a NON-forbidding sentence (parking it inside "do not use …" deletes the badge silently); to keep a card off Step-by-Step never write the hyphenated token (numbered "Step 1 / Step 2" is safe). Worked example + its 23 pinning tests: `agent/migrations/0201_add_voice_commands_section.py` and `agent/test_voice_commands_catalog.py`.
...
418. Open `.gemini/skills/tlamatini-daily-chat-test/harness/` and read `config.py`, `run_test.py`, `questions.py`, `wrapped_questions.py`, `qualify.py`.
