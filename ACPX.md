...
> **Current-state banner (Tlamatini 1.14.0, 2026-06-03).** This file is the original ACPX/Skills design walkthrough; its per-section numbers are an intentional **historical Phase-1 snapshot** (5 ACPX `@tool`s, 20 seed skills, 14 agent_ids, 57 visual agents). The system has since grown — the authoritative current counts are: **72 visual agent types**, **79 Multi-Turn tools** (20 base + 47 wrapped `chat_agent_*` + 12 ACPX/Skill), **27 skills**, and a **12-tool LLM-facing ACPX/Skill surface** (`acp_doctor`, `list_acp_agents`, `acp_spawn`, `acp_send`, `acp_send_and_wait`, `acp_kill`, `acp_transcript`, `acp_session_status`, `acp_list_sessions`, `acp_relay`, `list_skills`, `invoke_skill`). The `DEFAULT_ACP_AGENTS` agent_id registry remains **14** entries (§3.5). For the live surface read `CLAUDE.md` and `docs/claude/acpx.md`, not the historical numbers below.
