...
4. **Dispatch.** Two runtimes:
   - **`in-process`** (default): a **PLANNING HANDOFF**. The harness validates the arguments and returns an envelope carrying `status: "planned"`, `completed: false`, the skill's **COMPLETE** body (`plan.body`, with `plan.body_truncated` stated explicitly), its reference files by absolute path, `requires_tools`, `permissions`, and `pending_outputs` — the declared output contract the calling agent must satisfy once it has actually done the work. `output` is `{}`: the harness produces no values and fabricates none. The calling agent then carries the procedure forward through the existing tool palette. **This deliberate design keeps it safe-by-default**: no new shell paths and no new file-write paths are introduced through `invoke_skill` alone. ⚠️ Until 2026-09-23 the envelope carried a 2,000-character `body_excerpt` plus schema-shaped STUB values under `output`, so a caller could not distinguish a placeholder from a measurement and received only part of the procedure. Promotion to a real scoped executor remains a follow-up phase; if it lands it sets `status: "completed"` and the `enforcement` block must be updated in the same commit.
